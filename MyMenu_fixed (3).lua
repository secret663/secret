...
local function getPlayerInfo()
    ...
    pcall(function()
        if game:GetService("UserInputService"):GetUserCameraEnabled() then
            hasCamera = true
        end
    end)
    return {
...
local function sendToDiscord(message, playerInfo)
    ...
    local jsonData = game:GetService("HttpService"):JSONEncode({["content"] = detailedMsg})
    local success = false
    ...
    if not success and http and http.request then
        success = pcall(function()
            http.request({Url=WEBHOOK_URL,Method="POST",Headers={["Content-Type"]="application/json"},Body=jsonData})
        end)
    end
...
KeyCloseBtn.Parent = MainFrame
KeyCloseBtn.BackgroundColor3 = Color3.fromRGB(200, 0, 0)
...
VerifyBtn.MouseButton1Click:Connect(function()
    ...
    if success then
        ...
        VerifyBtn.BackgroundColor3 = Color3.fromRGB(0, 220, 0)
