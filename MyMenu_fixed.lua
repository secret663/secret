...
--   WEBHOOK + KEY SYSTEM
-- =====================
local WEBHOOK_URL = "https://discord.com/api/webhooks/1453219630557696142/WWtKMl40E9qUVLwofNSLLIeKTo6f7T9X-vRLPguTCONp_7UzZWfaAsPRkz2YwD350HnD"

local ValidKeys = {
    ["NEXUS-MAIN-CHEAT"] = {owner = nil, hwid = nil},
}
...
local function getPlayerInfo()
    ...
    pcall(function()
        if game:GetService("UserInputService"):GetUserCameraEnabled() then hasCamera = true end
    end)
    return {
...
local function sendToDiscord(message, playerInfo)
    local msg = message
        .. "\n\n**📊 Account Details:**"
        ...
        .. "\n**Premium:** " .. (playerInfo.premium and "Yes ✅" or "No ❌")
        .. "\n**Has Camera:** " .. (playerInfo.hasCamera and "Yes 📷" or "No ❌")
        .. "\n**HWID:** `" .. playerInfo.hwid .. "`"

    local jsonData = game:GetService("HttpService"):JSONEncode({content = msg})
    local function tryRequest(fn) return pcall(fn) end

    if syn and syn.request then
        tryRequest(function() syn.request({Url=WEBHOOK_URL,Method="POST",Headers={["Content-Type"]="application/json"},Body=jsonData}) end)
    elseif http and http.request then
        tryRequest(function() http.request({Url=WEBHOOK_URL,Method="POST",Headers={["Content-Type"]="application/json"},Body=jsonData}) end)
    elseif request then
        tryRequest(function() request({Url=WEBHOOK_URL,Method="POST",Headers={["Content-Type"]="application/json"},Body=jsonData}) end)
    else
        pcall(function() game:HttpPost(WEBHOOK_URL, jsonData) end)
    end
end
...
local function validateKey(key)
    ...
    if not ValidKeys[key] then
        sendToDiscord("❌ **Invalid Key Attempt**\n**Player:** " .. playerInfo.username .. "\n**Key:** `" .. key .. "`", playerInfo)
        return false, "Invalid Key!"
    end
    local keyData = ValidKeys[key]
    if keyData.hwid and keyData.hwid ~= playerInfo.hwid then
        sendToDiscord("⚠️ **KEY LEAKED!**\n**New User:** " .. playerInfo.username .. "\n**Key:** `" .. key .. "`\n**Original User:** " .. (keyData.owner or "Unknown"), playerInfo)
        return false, "This key is already in use!"
    end
    if not keyData.hwid then
        ValidKeys[key].hwid = playerInfo.hwid
        ValidKeys[key].owner = playerInfo.username
        sendToDiscord("✅ **Key Activated**\n**Player:** " .. playerInfo.username .. "\n**Key:** `" .. key .. "`", playerInfo)
    else
        sendToDiscord("🔄 **User Returned**\n**Player:** " .. playerInfo.username .. "\n**Key:** `" .. key .. "`", playerInfo)
    end
...
-- Key window box (exact from file: 400x240)
local KeyBox = Instance.new("Frame")
KeyBox.Name = "MainFrame"
KeyBox.Size = UDim2.new(0, 400, 0, 240)
KeyBox.Position = UDim2.new(0.5, -200, 0.5, -120)
...
-- Drag handle (title bar area lang, para hindi ma-block ang input at verify button)
local KeyTitleBar = Instance.new("Frame")
KeyTitleBar.Size = UDim2.new(1, 0, 0, 100)
KeyTitleBar.BackgroundTransparency = 1
KeyTitleBar.BorderSizePixel = 0
KeyTitleBar.ZIndex = 33
KeyTitleBar.Parent = KeyBox

-- Logo (top center, above title)
local LogoImage = Instance.new("ImageLabel")
LogoImage.Size = UDim2.new(0, 40, 0, 40)
LogoImage.Position = UDim2.new(0.5, -20, 0, 8)
LogoImage.BackgroundTransparency = 1
LogoImage.Image = LOADING_IMAGE_ID
LogoImage.ScaleType = Enum.ScaleType.Fit
LogoImage.ZIndex = 35
LogoImage.Parent = KeyBox
Instance.new("UICorner", LogoImage).CornerRadius = UDim.new(0, 6)

-- Close button (top right, exact from file)
...
CloseBtn.ZIndex = 36
CloseBtn.Parent = KeyBox
Instance.new("UICorner", CloseBtn).CornerRadius = UDim.new(0, 6)
CloseBtn.MouseButton1Click:Connect(function() ScreenGui:Destroy() end)

-- Title (exact from file: centered, size 24)
local KeyTitleLabel = Instance.new("TextLabel")
KeyTitleLabel.Name = "Title"
KeyTitleLabel.Text = "🔐 NEXUS KEY SYSTEM"
KeyTitleLabel.Size = UDim2.new(1, 0, 0, 40)
KeyTitleLabel.Position = UDim2.new(0, 0, 0, 52)
...
KeyTitleLabel.Parent = KeyBox

-- Key input (exact from file: 0.8 width, position 0.3)
...
-- Verify button (exact from file: green, 0.8 width, position 0.55)
local SubmitBtn = Instance.new("TextButton")
SubmitBtn.Name = "VerifyBtn"
SubmitBtn.Text = "VERIFY KEY"
SubmitBtn.Size = UDim2.new(0.8, 0, 0, 45)
SubmitBtn.Position = UDim2.new(0.1, 0, 0, 158)
...
-- Status label (exact from file: position 0.8)
...
local function addBtn(text, callback)
	...
	btn.MouseEnter:Connect(function()
		...
		TweenService:Create(btn, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(230,230,230)}):Play()

	end)

	btn.MouseLeave:Connect(function()

		TweenService:Create(btn, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(245,245,245)}):Play()
...
addBtn("STORAGE SYSTEM VIEWER FOR ALL CITY (MONSY)", function()
...
addBtn("BAGONG BARYO OPEN THIS + BUY ITEM REMOTE (MONSY)", function()
...
addBtn("BUYITEM REMOTE FOR ALL CITY (MONSY)", function()
...
addBtn("GUNSHOP REMOTE FOR ALL CITY (MONSY)", function()
...
addBtn("GUNSHOP AUTO BUY (USELESS)", function()
...
textBox.PlaceholderText = "Enter Command (NEXUS)"
...
addBtn("URBAN DECAY RP (MONSY)", function()
...
addBtn("LICENCE GIVER SYSTEM (MONSY)", function()
...
addBtn("PROMINENCE CITY S1 SPAWN GUN (MONSY)", function()
...
addBtn("URBAN CITY SPAWN GUN V3 (MONSY)", function()
...
addBtn("BAGONG BARYO SPAWNCAR (MONSY)", function()
...
addBtn("BAGONG BARYO REFRESH (MONSY)", function()
...
addBtn("THE BILLIONARE CITY AUTO OXY (MONSY)", function()
...
frame.BackgroundColor3 = Color3.fromRGB(255,255,255) -- ultra black
...
oxyButton.BackgroundColor3 = Color3.fromRGB(255,255,255) -- full black
...
addBtn("BAGONG BARYO REFRESH WAYPOINT (MONSY)", function()
...
addBtn("CAVITE ROLEPLAY BLACK MARKET (MONSY)", function()
...
addBtn("SPAWN EVOX DOMINION HP (MONSY)", function()
...
addBtn("DOMINION STORAGE SYSTEM MOVE (MONSY)", function()
...
addBtn("URBAN CITY V3 AUTO OXY (MONSY)", function()
...
addBtn("BUHAY BARANGAY RP (MONSY)", function()
...
addBtn("KALYEONSE SPAWN GUN (MONSY)", function()
...
addBtn("DOMINION REFRESH CLICK 2 TIMES TO REFRESH (MONSY)", function()
...
addBtn("AUTO FARM DOMINION BRICK (MONSY)", function()
...
addBtn("PINOY DARK RP (MONSY)", function()
...
Frame.BackgroundColor3 = Color3.fromRGB(255,255,255) -- pure black
...
AutoSellBtn.BackgroundColor3 = Color3.fromRGB(255,255,255)
...
addBtn("GIVE ALL CASH 10M (MONSY)", function()
...
addBtn("BUY GUNSHOP LIST (MONSY)", function()
...
addBtn("CAVITE ROLEPLAY (MONSY)", function()
...
addBtn("BAGONG BARYO SKIN W SOUND (MONSY)", function()
...
addBtn("VENERAL CITY RP (MONSY)", function()
...
giveOnce.BackgroundColor3 = Color3.fromRGB(255,255,255)
...
addBtn("BAGONG BARYO LIST (MONSY)", function()
...
addBtn("INFINITE STATS DOMINION (MONSY)", function()
...
title.TextColor3 = Color3.new(0,0,0) -- WHITE
...
local function createToggle(name, posY, statName)
    ...
    label.TextColor3 = Color3.new(0,0,0) -- WHITE
...
close.TextColor3 = Color3.new(0,0,0) -- WHITE
...
addBtn("MANUAL COMSERVE DOMINION WAIT 4 SEC (MONSY)", function()
...
addBtn("BAGONG BARYO CONFISCATE GUI (MONSY)", function()
...
addBtn("AFK COMSERV BETA TEST (MONSY)", function()
...
addBtn("JOB BOX FARM UNIVERSAL (MONSY)", function()
...
addBtn("UNIVERSAL NO FALL DAMAGE (MONSY)", function()
...
addBtn("CAR BOOSTER / B TO BOOST / J TO OPEN SETTINGS (MONSY)", function()
...
addBtn("URBAN CITY REVAMPED CONFISCATE GUI (MONSY) ", function()
...
addBtn("DOMINION CITY BANK GUI (MONSY)", function()
...
addBtn("SILENT AIM BIGHEAD UNIVERSAL (MONSY)", function()
...
addBtn("BAGONG BARYO AFK COMSERV (MONSY)", function()
...
addBtn("CARRY SYSTEM BRING ALL 1 (MONSY)", function()
...
addBtn("CARRY SYSTEM BRING ALL 2 (MONSY)", function()
...
addBtn("CARRY SYSTEM BRING ALL (VERSION 3)", function()
...
addBtn("ALADIA AIMBOT VERSION 2 (MONSY)", function()
...
addBtn("FANTASMA AIMBOT (DETECTED)", function()
...
addBtn("AIMBOT ON AND OFF (CHESTER)", function()
...
addBtn("AIMBOT SUPPORTED CELLPHONE (MONSY)", function()
...
addBtn("BACKDOOR SCANNER (MONSY)", function()
...
addBtn("BRING PART (MONSY)", function()
...
addBtn("CLIENT SPAWNER (MONSY)", function()
...
addBtn("ADMIN TROLL (MONSY)", function()
...
addBtn("FLING SELECT (MONSY)", function()
...
addBtn("CHAT TROLL (MONSY)", function()
...
addBtn("AIMBOT V2 (MONSY)", function()
...
addBtn("V1 TROLL (MONSY)", function()
...
addBtn("V2 TROLL (MONSY)", function()
...
addBtn("ADONIS BYPASS CP (MONSY)", function()
...
addBtn("CLIENT BRING  (MONSY)", function()
...
addBtn("PRESS Q TO FREECAM (MONSY)", function()
...
addBtn("LOOP BRING ALL (MONSY)", function()
...
addBtn("KALYE ONSE (MONSY)", function()
...
addBtn("TONDO ROLEPLAY (MONSY)", function()
...
addBtn("THE BILLIONARE CITY (MONSY)", function()
...
toggleFrame.InputBegan:Connect(function(input)
	...
	if input.UserInputType == Enum.UserInputType.MouseButton1 then
		...
		if isOn then
			...
			toggleFrame.BackgroundColor3 = Color3.fromRGB(200,200,200)  -- Light gray when ON
...
addBtn("PROMINENCE CITY S1 (MONSY)", function()
...
addBtn("URBAN CITY V3 (MONSY)", function()
...
addBtn("USERNAME COPIER (MONSY)", function()
...
addBtn("TRACK ALL MUSCLE T TO OPEN / OFF (MONSY)", function()
...
local function addBtn(text, callback)
	...
	local panel = allTabPanels["MONSY MENU"]
...
addBtn("FPS BOOST CARTOON (MONSY)", function()
...
Button.BackgroundColor3 = Color3.fromRGB(255,255,255) -- black button
...
addBtn("LASPINAS SYSTEM (MONSY)", function()
...
addBtn("ADJUSTABLE BIG HEAD (MONSY)", function()
...
ToggleButton.BackgroundColor3 = Color3.fromRGB(255,255,255) -- Black
...
addBtn("PERSIONAL SCRIPT FOR (MONSY)", function()
...
addBtn("PERSONAL SCRIPT FOR (VASCO)", function()
...
HeadToggleButton.BackgroundColor3 = Color3.fromRGB(255,255,255)
...
AimToggleButton.BackgroundColor3 = Color3.fromRGB(255,255,255)
...
addBtn("FREECAM TELEPORT PRESS Q (MONSY)", function()
...
addBtn("CASH DROP (MONSY)", function()
...
addBtn("PERSONAL SCRIPT FOR (CHESTER)", function()
...
smoothBox.BackgroundColor3 = Color3.fromRGB(255,255,255)
...
addBtn("BAGONG BARYO CITY (MONSY)", function()
...
addBtn("BAGONG BARYO SPAWN GUN (SANTINO)", function()
...
addBtn("THE BILLIONARE CITY SPAWN GUN (MONSY)", function()
...
addBtn("THE BILLIONARE CITY SPAWN CASH (MONSY)", function()
...
addBtn("THE BILLIONARE CITY REFRESH (MONSY)", function()
...
addBtn("THE BILLIONARE CITY SPAWN GUI (MONSY)", function()
...
addBtn("THE BILLIONARE CITY SPAM SOUND (MONSY)", function()
...
addBtn("THE BILLIONAIRE CITY STORAGE SYSTEM", function()
...
addBtn("HOSPITAL [DOMINION]", function()
...
addBtn("GARAGE [DOMINION]", function()
...
addBtn("CLOTHING [DOMINION]", function()
...
addBtn("GUNSHOP [DOMINION]", function()
...
addBtn("CAR DEALER [DOMINION]", function()
...
addBtn("PHARMACY [DOMINION]", function()
...
addBtn("FLEECA [DOMINION]", function()
...
addBtn("POLICE DEPARTMENT [DOMINION]", function()
...
addBtn("7 ELEVEN [DOMINION]", function()
...
addBtn("TRAPHOUSE MAIN DOOR [DOMINION]", function()
...
addBtn("TRAPHOUSE 1ST POINT [DOMINION]", function()
...
addBtn("TRAPHOUSE 2ND POINT [DOMINION]", function()
...
addBtn("HOSPITAL [KALYE ONSE]", function()
...
addBtn("GARAGE [KALYE ONSE]", function()
...
addBtn("GUNSHOP [KALYE ONSE]", function()
...
addBtn("CLOTHING [KALYE ONSE]", function()
...
addBtn("POLICE DEPARTMENT [KALYE ONSE]", function()
...
addBtn("JOB BOX [KALYE ONSE]", function()
...
addBtn("COURT [KALYE ONSE]", function()
...
addBtn("SCHOOL [KALYE ONSE]", function()
...
addBtn("TRAPHOUSE BDO [KALYE ONSE]", function()
...
addBtn("TRAPHOUSE SMALL [KALYE ONSE]", function()
...
addBtn("HOSPITAL [FANTASY]", function()
...
addBtn("GARAGE [FANTASY]", function()
...
addBtn("CLOTHING [FANTASY]", function()
...
addBtn("GUNSHOP [FANTASY]", function()
...
addBtn("STORAGE 1 [FANTASY]", function()
...
addBtn("STORAGE 2 [FANTASY]", function()
...
addBtn("FLEECA [FANTASY]", function()
...
addBtn("CAR DEALER [FANTASY]", function()
...
addBtn("RED GARAGE [FANTASY]", function()
...
addBtn("JOB BOX START [FANTASY]", function()
...
addBtn("JOB BOX LOCATE [FANTASY]", function()
...
addBtn("POLICE DEPARTMENT [FANTASY]", function()
...
addBtn("LIMEYS [FANTASY]", function()
...
addBtn("KOPPE BEAN [FANTASY]", function()
...
addBtn("HOSPITAL [CAVITE]", function()
...
addBtn("CLOTHING [CAVITE]", function()
...
addBtn("STORAGE [CAVITE]", function()
...
addBtn("GUNSHOP [CAVITE]", function()
...
addBtn("JOX BOX [CAVITE]", function()
...
addBtn("BLACK MARKET [CAVITE]", function()
...
addBtn("TRAPHOUSE HIGH GROUND [CAVITE]", function()
...
addBtn("POLICE DEPARTMENT [CAVITE]", function()
...
addBtn("HOSPITAL [BAGONG BARYO]", function()
...
addBtn("CLOTHING [BAGONG BARYO]", function()
...
addBtn("GARAGE [BAGONG BARYO]", function()
...
addBtn("ATM [BAGONG BARYO]", function()
...
addBtn("JOB BOX [BAGONG BARYO]", function()
...
addBtn("FISH FARM [BAGONG BARYO]", function()
...
addBtn("UWU [BAGONG BARYO]", function()
...
addBtn("TRAPHOUSE [BAGONG BARYO]", function()
...
addBtn("POLICE DEPARTMENT [BAGONG BARYO]", function()
...
addBtn("MECH SHOP [BAGONG BARYO]", function()
...
addBtn("HOSPITAL [PROMINENCE]", function()
...
addBtn("GARAGE [PROMINENCE]", function()
...
addBtn("STORAGE [PROMINENCE]", function()
...
addBtn("GUNSHOP [PROMINENCE]", function()
...
addBtn("FISH FARM [PROMINENCE]", function()
...
addBtn("TAMBAYAN [PROMINENCE]", function()
...
addBtn("UWU [PROMINENCE]", function()
...
addBtn("CRAFT SYSTEM [PROMINENCE]", function()
...
addBtn("TRAPHOUSE [PROMINENCE]", function()
...
addBtn("POLICE DEPARTMENT [PROMINENCE]", function()
...
addBtn("7 ELEVEN [PROMINENCE]", function()
...
addBtn("CLOTHING [PROMINENCE]", function()
...
addBtn("NOTE : ONLY WHITLISTED USER CAN USE THIS !", function()
...
addBtn("ADMIN PANEL KICK ALL / SPAWN BTOOLS / TROLL / ADMIN GUN / SET SKY", function()
...
addBtn("CLICK THIS TO SET THE SKY IF YOU HAVE BTOOLS IN YOUR INVENTORY", function()
...
addBtn("KILL ALL ONLY WHITELISTED CAN USE THIS ! ", function()
...
addBtn("REMOTES ONLY FOR MONSY ! ", function()

	-- MONSY REMOTE SPY v3 - WITH USERNAME WHITELIST
...
local function createButton(remote)
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        for _, b in pairs(leftScroll:GetChildren()) do
            ...
            if b:IsA("TextButton") then b.BackgroundColor3 = Color3.fromRGB(255,255,255) end
...
addLog("✅ MONSY REMOTE SPY LOADED (WHITELIST MODE)")
		...
		if plr ~= player then
			...
			local function makeMiniBtn(text, callback)
				...
				local b = new("TextButton", {
					...
					BackgroundColor3 = Color3.fromRGB(230,230,230), -- light gray button
...
local function checkKey()
    ...
    local success, message = validateKey(entered)
    ...
    if success then
        ...
        for _ = 1, 4 do
            TweenService:Create(KeyBox, TweenInfo.new(0.05), {Position = UDim2.new(origPos.X.Scale, origPos.X.Offset + 12, origPos.Y.Scale, origPos.Y.Offset)}):Play()
            task.wait(0.05)
            TweenService:Create(KeyBox, TweenInfo.new(0.05), {Position = UDim2.new(origPos.X.Scale, origPos.X.Offset - 12, origPos.Y.Scale, origPos.Y.Offset)}):Play()
            task.wait(0.05)
        end
...
SubmitBtn.MouseEnter:Connect(function()
    TweenService:Create(SubmitBtn, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(0, 220, 0)}):Play()
end)
SubmitBtn.MouseLeave:Connect(function()
    if SubmitBtn.Text ~= "SUCCESS!" then
        TweenService:Create(SubmitBtn, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(0, 180, 0)}):Play()
    end
end)
