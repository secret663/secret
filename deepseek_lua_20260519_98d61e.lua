...
local themes = {
	BLACK = {primary = Color3.fromRGB(255,255,255), accent = Color3.fromRGB(80,80,80)},
}
...
-- ALL 14 TOP TABS (complete from original)
local topTabNames = {
	"MAIN MENU", "ROLEPLAY MENU", "ROLEPLAY MENU 2", "LATEST SCRIPT",
	"BRING MENU", "AIMBOT MENU", "SCRIPT MENU", "CASH MENU",
	"PLAYER MENU", "NEXUS MENU", "THE BILLIONARE", "RP TELEPORT",
	"ADMIN PANEL", "REMOTES"
}
...
-- BUTTON CREATOR FUNCTION FOR ALL TABS
...
-- ============ MAIN MENU BUTTONS (COMPLETE) ============
local mainPanel = allTabPanels["MAIN MENU"]

addButton(mainPanel, "INFINITE YIELD (NEXUS)", function()
	loadstring(game:HttpGet("https://raw.githubusercontent.com/EdgeIY/infiniteyield/master/source"))()
end)

addButton(mainPanel, "MUSIC EXPLOITS (NEXUS)", function()
...
addButton(mainPanel, "ESP PLAYER (NEXUS)", function()
	local Players = game:GetService("Players")
	local RunService = game:GetService("RunService")
...
addButton(mainPanel, "BIG HEAD PLAYER (NEXUS)", function()
	_G.HeadSize = 6 
	_G.Disabled = true 
...
addButton(mainPanel, "KEYBIND Q TO FREECAM (NEXUS)", function()
	local Players = game:GetService("Players")
	local UserInputService = game:GetService("UserInputService")
	local RunService = game:GetService("RunService")
	local player = Players.LocalPlayer
	...
	local function addKeyDirection(key)
		...
		if key == Enum.KeyCode.LeftControl then moveDir += Vector3.new(0,-1,0) end
	end
...
addButton(mainPanel, "AIMBOT ADJUSTABLE (NEXUS)", function()
	getgenv().Environment = {
...
addButton(mainPanel, "FADED GUI UNIVERSAL (NEXUS)", function()
	loadstring(game:HttpGet("https://raw.githubusercontent.com/NighterEpic/Faded-Grid/main/YesEpic", true))()
end)

addButton(mainPanel, "LOOP BRING ALL (NEXUS)", function()
	local Players = game:GetService("Players")
	local RunService = game:GetService("RunService")
	local LocalPlayer = Players.LocalPlayer
...
addButton(mainPanel, "PLAYER SCANNER (NEXUS)", function()
	local Players = game:GetService("Players")
	local LocalPlayer = Players.LocalPlayer
	local UIS = game:GetService("UserInputService")
...
addButton(mainPanel, "AIM VIEWER (NEXUS)", function()
	local Players = game:GetService("Players")
	local LocalPlayer = Players.LocalPlayer
	local RunService = game:GetService("RunService")
	local Camera = workspace.CurrentCamera
...
addButton(mainPanel, "HEAD DETECTOR (NEXUS)", function()
	local Players = game:GetService("Players")
	local LocalPlayer = Players.LocalPlayer
	local UserInputService = game:GetService("UserInputService")
...
addButton(mainPanel, "PLAYER LIST TP (NEXUS)", function()
	local Players = game:GetService("Players")
	local RunService = game:GetService("RunService")
	local UserInputService = game:GetService("UserInputService")
	local LocalPlayer = Players.LocalPlayer
...
addButton(mainPanel, "TPUNANCHORED TO PLAYER (NEXUS)", function()
	local Players = game:GetService("Players")
	local LocalPlayer = Players.LocalPlayer
	local Workspace = game:GetService("Workspace")
	...
	local function isToolAllowed(tool)
		...
		elseif filterModes[filterIndex] == "Melee Only" then
			return name:find("knife") or name:find("bat") or name:find("blade") or name:find("crowbar")
		end
		return false
	end
...
addButton(mainPanel, "FLING ALL (NEXUS)", function()
...
addButton(mainPanel, "HITBOX EXTENDER (NEXUS)", function()
...
print("NEXUS MENU LOADED - COMPLETE VERSION")