...
local settingsTable = {
	General = {
		...
		rayfieldOpen = {Type = 'bind', Value = 'K', Name = 'NEXUS Keybind'},
		-- buildwarnings
		-- rayfieldprompts
		...
		if useStudio then
			...
			task.spawn(function()
				...
				if success and reporter then
					pcall(function()
						reporter.report("NEXUS", Release, InterfaceBuild)
					end)
				else
...
SNTS = {
	...
	BackgroundImage = "rbxassetid://108700890668070", 
	Background = Color3.fromRGB(15, 15, 15), -- Fallback color if no image
	...
	SliderProgress = Color3.fromRGB(100, 100, 100),
	...
	ToggleEnabledStroke = Color3.fromRGB(0, 255, 120),
	ToggleDisabledStroke = Color3.fromRGB(100, 100, 100),
	ToggleEnabledOuterStroke = Color3.fromRGB(0, 100, 50),
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('STUB Settings', 0, true)

	if TabList['STUB Settings'] then
		TabList['STUB Settings'].LayoutOrder = 1000
	end

	if Elements['STUB Settings'] then
		Elements['STUB Settings'].LayoutOrder = 1000
	end
...
function RayfieldLibrary:CreateWindow(Settings)
	...
	if Settings.LoadingTitle ~= "Rayfield Interface Suite" then
		LoadingFrame.Version.Text = "STUB UI"
	end
		...
		task.spawn(function()
			while true do
				...
				RayfieldLibrary:Notify({
					Title = "Stubborn Interface",
					Content = "Enjoying this UI library? Find it at https://discord.gg/FdPsV6K6",
					Duration = 7,
					Image = 4370033185,
				})
			end
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['STUB Settings'])
		end)
	end)
...
if useStudio then
	...
	local Window = RayfieldLibrary:CreateWindow({
		Name = "NEXUS V1.0",
		LoadingTitle = "NEXUS V1.0",
		Theme = 'Default',
		Icon = 0,
		LoadingSubtitle = "by secret663",
		ConfigurationSaving = {
			Enabled = true,
		...
		KeySettings = {
			...
			Note = "Key: NEXUS-MAIN-CHEAT",
			FileName = "NexusKey",
			SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
			GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like Rayfield to get the key from
			Key = {"maisan is here nigga"}
		}
	})
