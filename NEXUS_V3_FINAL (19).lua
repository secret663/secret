...
-- Anti cheat starts when key system loads
task.spawn(function()


local _ac_Players = game:GetService("Players")
local _ac_TweenService = game:GetService("TweenService")
local _ac_player = _ac_Players.LocalPlayer
...
local _ac_folderKeywords = {
    ...
    "antivoid","anti_void","hey","security",
    "secure","protection","protect",
    "shield","guard","checker","detector",
}

local function _ac_isSuspiciousFolder(obj)
    if not (obj:IsA("Folder") or obj:IsA("Model") or obj:IsA("Configuration")) then return false end
    local name = obj.Name:lower()
    for _, kw in ipairs(_ac_folderKeywords) do
        if name:find(kw, 1, true) then return true end
    end
    return false
end

local function _ac_isAntiCheat(obj)
    if not (obj:IsA("LocalScript") or obj:IsA("Script") or obj:IsA("ModuleScript")) then return false end
    local name = obj.Name:lower()
    local nameKeywords = {
        "sluzanti","sluzan","securev","anticheat",
        "anti_cheat","antiexploit","anti_exploit",
        "antihack","anti_hack","antitamper",
        "kickscript","autokick","cheatdetect",
        "exploitdetect","hackdetect","morrix","hey","main",
    }
    ...
    local hasKick = src:find("player:kick") or src:find(":kick(") or src:find("players:kick")
    if not hasKick then return false end
    local patterns = {
        "checkcaller","identifyexecutor","getexecutorname",
        "isexecutorclosure","getrenv","getgenv",
        "getrawmetatable","hookfunction","hookmetamethod",
        "unauthorized","third.party","3rd.party",
        "exploit","executor","injected",
        "cheat detected","hack detected",
        "hitbox","headexpander","sluzanti","sluzan","securev",
        "anticheat","antiexploit","error code: 267",
        "antivoid","anti_void","void detection",
    }
...
local function _ac_checkAndDestroy(obj)
    if _ac_isSuspiciousFolder(obj) or _ac_isAntiCheat(obj) then
        pcall(function() obj:Destroy() _ac_destroyed += 1 end)
        return true
    end
    return false
end

local function _ac_scanLocation(parent)
    if not parent then return end
    pcall(function()
        for _, obj in ipairs(parent:GetChildren()) do
            if _ac_isSuspiciousFolder(obj) then
                pcall(function() obj:Destroy() _ac_destroyed += 1 end)
            else
                if _ac_isAntiCheat(obj) then
                    pcall(function() obj:Destroy() _ac_destroyed += 1 end)
                end
                pcall(function()
                    for _, child in ipairs(obj:GetChildren()) do
                        _ac_checkAndDestroy(child)
                    end
                end)
            end
            task.wait()
...
local function _ac_destroyAntiCheat()
    local before = _ac_destroyed
    _ac_scanLocation(_ac_player.PlayerGui)
    _ac_scanLocation(_ac_player:FindFirstChild("PlayerScripts"))
    _ac_scanLocation(game:GetService("StarterPlayer"):FindFirstChild("StarterPlayerScripts"))
    _ac_scanLocation(game:GetService("StarterPlayer"):FindFirstChild("StarterCharacterScripts"))
    _ac_scanLocation(game:GetService("ReplicatedStorage"))
    _ac_scanLocation(game:GetService("ReplicatedFirst"))
    _ac_scanLocation(workspace)
    return _ac_destroyed - before
end

-- Notif GUI
local _ac_gui = Instance.new("ScreenGui")
_ac_gui.Name = "AntiCheatDestroyer"
_ac_gui.ResetOnSpawn = false
_ac_gui.DisplayOrder = 3000
_ac_gui.Parent = _ac_player:WaitForChild("PlayerGui")

local _ac_notif = Instance.new("Frame")
_ac_notif.Size = UDim2.new(0, 220, 0, 44)
_ac_notif.Position = UDim2.new(1, 260, 1, -80)
_ac_notif.BackgroundColor3 = Color3.fromRGB(8, 8, 8)
_ac_notif.BorderSizePixel = 0
_ac_notif.Parent = _ac_gui
Instance.new("UICorner", _ac_notif).CornerRadius = UDim.new(0, 6)
Instance.new("UIStroke", _ac_notif).Color = Color3.fromRGB(30, 30, 30)

local _ac_redBar = Instance.new("Frame", _ac_notif)
_ac_redBar.Size = UDim2.new(0, 4, 1, 0)
_ac_redBar.BackgroundColor3 = Color3.fromRGB(200, 30, 30)
_ac_redBar.BorderSizePixel = 0
Instance.new("UICorner", _ac_redBar).CornerRadius = UDim.new(0, 6)

local _ac_redBarFill = Instance.new("Frame", _ac_notif)
_ac_redBarFill.Size = UDim2.new(0, 4, 1, 0)
_ac_redBarFill.Position = UDim2.new(0, 2, 0, 0)
_ac_redBarFill.BackgroundColor3 = Color3.fromRGB(200, 30, 30)
_ac_redBarFill.BorderSizePixel = 0

local _ac_notifTitle = Instance.new("TextLabel", _ac_notif)
_ac_notifTitle.Size = UDim2.new(1, -14, 0, 20)
_ac_notifTitle.Position = UDim2.new(0, 10, 0, 4)
_ac_notifTitle.BackgroundTransparency = 1
_ac_notifTitle.Text = "ANTI CHEAT DESTROYED"
_ac_notifTitle.TextColor3 = Color3.fromRGB(220, 220, 220)
_ac_notifTitle.Font = Enum.Font.GothamBold
_ac_notifTitle.TextSize = 11
_ac_notifTitle.TextXAlignment = Enum.TextXAlignment.Left

local _ac_notifSub = Instance.new("TextLabel", _ac_notif)
_ac_notifSub.Size = UDim2.new(1, -14, 0, 18)
_ac_notifSub.Position = UDim2.new(0, 10, 0, 22)
_ac_notifSub.BackgroundTransparency = 1
_ac_notifSub.Text = "-nexus"
_ac_notifSub.TextColor3 = Color3.fromRGB(100, 100, 100)
_ac_notifSub.Font = Enum.Font.Gotham
_ac_notifSub.TextSize = 11
_ac_notifSub.TextXAlignment = Enum.TextXAlignment.Left

local _ac_notifActive = false
local function _ac_showNotif()
    if _ac_notifActive then return end
    _ac_notifActive = true
    _ac_TweenService:Create(_ac_notif, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
        Position = UDim2.new(1, -230, 1, -80)
    }):Play()
    task.wait(3)
    _ac_TweenService:Create(_ac_notif, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
        Position = UDim2.new(1, 260, 1, -80)
    }):Play()
    task.wait(0.3)
    _ac_notifActive = false
end

-- Auto scan every 2 seconds
end) -- end anti cheat task.spawn


local keySound = Instance.new("Sound")
keySound.SoundId = "rbxassetid://17208361335"
keySound.Volume = 1
keySound.Parent = keyGui
keySound:Play()

local keyFrame = Instance.new("Frame")
keyFrame.Size = UDim2.new(0, 420, 0, 270)
keyFrame.Position = UDim2.new(0.5, -210, 0.5, -135)
...
skullBg.Image = "rbxassetid://112389474191810"
skullBg.ScaleType = Enum.ScaleType.Crop
skullBg.ImageTransparency = 0.82
skullBg.ZIndex = 1
skullBg.Active = false
skullBg.Parent = keyFrame

-- Title
local keyTitle = Instance.new("TextLabel")
keyTitle.Size = UDim2.new(1, -20, 0, 36)
keyTitle.Position = UDim2.new(0, 14, 0, 14)
keyTitle.BackgroundTransparency = 1
keyTitle.Text = "NEXUS V4 — KEY SYSTEM"
...
keySub.Position = UDim2.new(0, 14, 0, 50)
keySub.BackgroundTransparency = 1
keySub.Text = "Enter your key to continue"
keySub.Font = Enum.Font.Gotham
keySub.TextSize = 13
keySub.TextColor3 = Color3.fromRGB(100, 100, 100)
...
divLine.Position = UDim2.new(0, 14, 0, 76)
divLine.BackgroundColor3 = Color3.fromRGB(22, 22, 22)
divLine.BorderSizePixel = 0
divLine.ZIndex = 3
divLine.Parent = keyFrame

-- Key input box
local keyBox = Instance.new("TextBox")
keyBox.Size = UDim2.new(1, -28, 0, 46)
keyBox.Position = UDim2.new(0, 14, 0, 90)
...
errorLbl.Size = UDim2.new(1, -28, 0, 20)
errorLbl.Position = UDim2.new(0, 14, 0, 144)
errorLbl.BackgroundTransparency = 1
errorLbl.Text = ""
errorLbl.Font = Enum.Font.Gotham
errorLbl.TextSize = 13
errorLbl.TextColor3 = Color3.fromRGB(255, 70, 70)
errorLbl.TextXAlignment = Enum.TextXAlignment.Left
errorLbl.ZIndex = 5
errorLbl.Parent = keyFrame

-- Enter button
local enterBtn = Instance.new("TextButton")
enterBtn.Size = UDim2.new(1, -28, 0, 50)
enterBtn.Position = UDim2.new(0, 14, 0, 170)
enterBtn.BackgroundColor3 = Color3.fromRGB(12, 12, 12)
...
local btnStroke = Instance.new("UIStroke", enterBtn)
btnStroke.Color = Color3.fromRGB(45, 45, 45)
btnStroke.Thickness = 1

enterBtn.MouseEnter:Connect(function()
	TweenService:Create(enterBtn, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(22, 22, 22)}):Play()
end)
enterBtn.MouseLeave:Connect(function()
	TweenService:Create(enterBtn, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(12, 12, 12)}):Play()
end)

-- Draggable (title area only, does NOT block button/textbox)
local kDragging, kDragStart, kStartPos = false, nil, nil
keyFrame.InputBegan:Connect(function(input)
	if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
		-- Only drag from the top 75px (title area)
		local relY = input.Position.Y - keyFrame.AbsolutePosition.Y
		if relY <= 75 then
			kDragging = true
...
UserInputService.InputChanged:Connect(function(input)
	if kDragging and kDragStart and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
		local d = input.Position - kDragStart
		keyFrame.Position = UDim2.new(kStartPos.X.Scale, kStartPos.X.Offset + d.X, kStartPos.Y.Scale, kStartPos.Y.Offset + d.Y)
	end
end)

local keySystemDone = false

local function onKeySubmit()
	if keySystemDone then return end
	if keyBox.Text == CorrectKey then
		keySystemDone = true
		keyGui:Destroy()

		-- Loading screen then show GUI
		task.defer(function()
			repeat task.wait() until _G.NexusScreenGui ~= nil
			...
			Shine.Size = UDim2.new(0.18, 0, 1.3, 0)
			Shine.Position = UDim2.new(-0.25, 0, -0.15, 0)
			...
			DotsFolder.Name = "FallenDots"
			DotsFolder.Parent = LoadGui
			...
			task.spawn(function()
				...
				local progressTween = TweenService2:Create(ProgressFill, TweenInfo.new(4.5, Enum.EasingStyle.Linear), {Size = UDim2.new(1, 0, 1, 0)})
				progressTween:Play()

				task.spawn(function()
					while ProgressFill.Parent do
						Shine.Position = UDim2.new(-0.25, 0, -0.15, 0)
						local shineTween = TweenService2:Create(Shine, TweenInfo.new(1.1, Enum.EasingStyle.Linear), {Position = UDim2.new(1.15, 0, -0.15, 0)})
						shineTween:Play()
				...
				task.spawn(function()
					while LoadGui.Parent and ProgressFill.Size.X.Scale < 0.95 do
						for _ = 1, 3 do
							local dot = Instance.new("Frame")
							dot.Size = UDim2.new(0, 6, 0, 6)
							dot.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
							dot.BackgroundTransparency = 0.4
							dot.Position = UDim2.new(math.random(5, 95)/100, 0, -0.1, 0)
							dot.AnchorPoint = Vector2.new(0.5, 0.5)
							dot.ZIndex = 6
							dot.Parent = DotsFolder
							Instance.new("UICorner", dot).CornerRadius = UDim.new(1, 0)
							TweenService2:Create(dot, TweenInfo.new(math.random(16, 28)/10, Enum.EasingStyle.Linear), {
								Position = UDim2.new(dot.Position.X.Scale, 0, 1.3, 0),
								BackgroundTransparency = 1
							}):Play()
							Debris2:AddItem(dot, 3.5)
						end
						task.wait(0.07)
					end
				end)
				...
				for i = 1, 20 do
					...
					ProgressBarBg.BackgroundTransparency = alpha
					ProgressFill.BackgroundTransparency = alpha
					Shine.BackgroundTransparency = alpha + 0.3
					task.wait(0.025)
				end

				LoadGui:Destroy()
				task.wait(0.5)
				_G.NexusScreenGui.Enabled = true
			end)
		end)
	else
		errorLbl.Text = "❌ Wrong key! Try again."
		TweenService:Create(keyFrame, TweenInfo.new(0.05, Enum.EasingStyle.Bounce), {Position = UDim2.new(keyFrame.Position.X.Scale, keyFrame.Position.X.Offset + 6, keyFrame.Position.Y.Scale, keyFrame.Position.Y.Offset)}):Play()
		task.wait(2)
		errorLbl.Text = ""
	end
end

enterBtn.MouseButton1Click:Connect(onKeySubmit)
...
local themes = {
	...
	WHITE = {primary = Color3.fromRGB(255,255,255), accent = Color3.fromRGB(35,35,35)},

	RED   = {primary = Color3.fromRGB(255,255,255), accent = Color3.fromRGB(255,50,50)},

	BLUE  = {primary = Color3.fromRGB(255,255,255), accent = Color3.fromRGB(50,150,255)},

	GRAY  = {primary = Color3.fromRGB(255,255,255), accent = Color3.fromRGB(35,35,35)},

	PINK  = {primary = Color3.fromRGB(255,255,255), accent = Color3.fromRGB(255,100,200)},
...
task.spawn(function()
	while main and main.Parent do
		...
		dot.Position = UDim2.new(math.random(0,100)/100,0,-0.05,0)
		dot.BackgroundColor3 = Color3.fromRGB(255,255,255)
		...
		dot.Parent = main
		Instance.new("UICorner",dot).CornerRadius = UDim.new(1,0)
		TweenService:Create(dot,TweenInfo.new(math.random(40,80)/10,Enum.EasingStyle.Linear),{
			Position = UDim2.new(dot.Position.X.Scale,0,1.05,0),
			BackgroundTransparency = 0.95,
		}):Play()
		task.delay(8,function() if dot and dot.Parent then dot:Destroy() end end)
	end
end)
...
new("TextLabel", {
	...
	Text = "discord.gg/NPPFejD8y\n\n@discord.gg/NPPFejD8y\n\n@\n\n@",
...
local function showTab(name)
	...
	if divider then divider.BackgroundColor3 = Color3.fromRGB(8,8,8) end

	if ring then ring.BackgroundColor3 = Color3.fromRGB(8,8,8) end
...
local topTabNames = {
	...
	"NEXUS V4", "THE BILLIONARE", "RP TELEPORT", "ADMIN PANEL", "REMOTES"
...
local function createTopTab(name)
	...
	btn.MouseEnter:Connect(function()
		...
		if currentTab ~= name then
			...
			TweenService:Create(btn, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(12,12,12)}):Play()
	...
	btn.MouseLeave:Connect(function()
		...
		if currentTab ~= name then
			...
			TweenService:Create(btn, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(8,8,8)}):Play()
...
if allTabButtons[1] then
	...
	allTabButtons[1].BackgroundColor3 = Color3.fromRGB(25,25,25)
...
addMainMenuButton("MUSIC EXPLOITS (NXS)", function()

	-- NXS CMD - Minimal FE Audio Player (White Font Update)
...
inputBox.PlaceholderText = "Enter Music ID [NXS]"
...
addMainMenuButton("KEYBIND Q TO FREECAM (NXS)", function()
...
addMainMenuButton("AIMBOT ADJUSTABLE (NXS)", function()
...
Frame.BackgroundColor3 = Color3.fromRGB(15,15,15) -- All black
...
Title.Text = "NXS AIMBOT" -- Custom Title
...
SliderBack.BackgroundColor3 = Color3.fromRGB(15,15,15) -- All black
...
StatusButton.BackgroundColor3 = Color3.fromRGB(15,15,15) -- black
...
addMainMenuButton("PLAYER SCANNER (NXS)", function()
...
ScanButton.BackgroundColor3 = Color3.fromRGB(8,8,8)
...
addMainMenuButton("AIM VIEWER (NXS)", function()
...
Frame.BackgroundColor3 = Color3.fromRGB(15,15,15) -- black bg
...
Title.BackgroundColor3 = Color3.fromRGB(15,15,15) -- black title bg

Title.Text = "NXS AIM VIEWER"
...
ToggleButton.BackgroundColor3 = Color3.fromRGB(15,15,15) -- black bg
...
addMainMenuButton("HEAD DETECTOR (NXS)", function()
...
addMainMenuButton("PLAYER LIST TP (NXS)", function()

	-- NXS - Player Viewer (LocalScript sa StarterGui)
...
addMainMenuButton("TPUNANCHORED TO PLAYER (NXS)", function()
...
UsernameBox.BackgroundColor3 = Color3.fromRGB(15,15,15)
...
FilterDropdown.BackgroundColor3 = Color3.fromRGB(15,15,15)
...
GrabButton.BackgroundColor3 = Color3.fromRGB(15,15,15)
...
local function addMainMenuButton(text, callback)
	...
	local panel = allTabPanels["ROLEPLAY MENU"]
...
addMainMenuButton("STORAGE SYSTEM VIEWER FOR ALL CITY (NXS)", function()
...
if not storageSystem then
    ...
    warn("❌ 'StorageSystem' not found in ReplicatedStorage.")
...
scrolling.BackgroundColor3 = Color3.fromRGB(22,22,22)
...
print("✅ StorageSystem Tools Viewer loaded (Press O to toggle, click to copy).")
...
addMainMenuButton("BAGONG BARYO OPEN THIS + BUY ITEM REMOTE (NXS)", function()
...
addMainMenuButton("BUYITEM REMOTE FOR ALL CITY (NXS)", function()
...
itemBox.BackgroundColor3 = Color3.fromRGB(15,15,15)
...
amountBox.BackgroundColor3 = Color3.fromRGB(15,15,15)
...
addMainMenuButton("GUNSHOP REMOTE FOR ALL CITY (NXS)", function()

    -- LocalScript: NXS CASH MENU (GunShop Purchase System)
...
addMainMenuButton("GUNSHOP AUTO BUY (NXS)", function()
...
textBox.PlaceholderText = "Enter Command (3NXS)"
...
addMainMenuButton("URBAN DECAY RP (NXS)", function()
...
titleLabel.Text = "NXS URBAN GUN BUY"
...
addMainMenuButton("LICENCE GIVER SYSTEM (NXS)", function()
...
addMainMenuButton("PROMINENCE CITY S1 SPAWN GUN (NXS)", function()
...
addMainMenuButton("URBAN CITY SPAWN GUN V3 (NXS)", function()
...
local items = {
    ...
    {"Suppressed Pistols", "Suppressed Pistols"},
...
local function addMainMenuButton(text, callback)
	...
	local panel = allTabPanels["ROLEPLAY MENU 2"]
...
addMainMenuButton("BAGONG BARYO SPAWNCAR (NXS)", function()
...
addMainMenuButton("BAGONG BARYO REFRESH (NXS)", function()
...
title.BackgroundTransparency = 1                  -- WALANG BACKGROUND

title.Text = "Auto Refresh"

title.TextColor3 = Color3.fromRGB(255,255,255)               -- BLACK TEXT
...
-- Refresh Button — LIGHT GRAY (para kita) + BLACK TEXT
...
refreshButton.BackgroundColor3 = Color3.fromRGB(230, 230, 230)  -- LIGHT GRAY

refreshButton.Text = "Refresh"

refreshButton.TextColor3 = Color3.fromRGB(255,255,255)                    -- BLACK TEXT
...
refreshButton.BorderColor3 = Color3.fromRGB(40,40,40)                 -- BLACK BORDER
...
-- Hover effect (mas maganda pa rin)
...
addMainMenuButton("THE BILLIONARE CITY AUTO OXY (NXS)", function()
...
frame.BackgroundColor3 = Color3.fromRGB(15,15,15) -- ultra black
...
oxyButton.BackgroundColor3 = Color3.fromRGB(15,15,15) -- full black
...
addMainMenuButton("BAGONG BARYO REFRESH WAYPOINT (NXS)", function()
...
-- Function to create buttons (WHITE THEME)
...
local function createButton(text, order)
    ...
    button.BackgroundColor3 = Color3.fromRGB(240, 240, 240)  -- LIGHT GRAY (para kita)

    button.TextColor3 = Color3.fromRGB(255,255,255)                    -- BLACK TEXT
    ...
    button.BorderColor3 = Color3.fromRGB(40,40,40)                  -- BLACK BORDER
...
addMainMenuButton("CAVITE ROLEPLAY BLACK MARKET (NXS)", function()
...
amountBox.PlaceholderText = "Enter Amount (NXS CMD)"
...
-- 🧠 NEW SYSTEM (replaces old Purchase event)
...
addMainMenuButton("SPAWN EVOX DOMINION HP (NXS)", function()
...
addMainMenuButton("DOMINION STORAGE SYSTEM MOVE (NXS)", function()
...
modeButton.BackgroundColor3 = Color3.fromRGB(15,15,15)
...
qtyBox.BackgroundColor3 = Color3.fromRGB(15,15,15)
...
moveButton.BackgroundColor3 = Color3.fromRGB(15,15,15)
...
addMainMenuButton("URBAN CITY V3 AUTO OXY (NXS)", function()
...
addMainMenuButton("BUHAY BARANGAY RP (NXS)", function()
...
addMainMenuButton("KALYEONSE SPAWN GUN (NXS)", function()
...
addMainMenuButton("DOMINION REFRESH CLICK 2 TIMES TO REFRESH (NXS)", function()
...
-- IMAGE (PALITAN MO YUNG ASSET ID DITO)

local imageId = "rbxassetid://112389474191810" -- Your Image



-- GUI CREATION (FULL SCREEN IMAGE 4 SECONDS)
...
screen.IgnoreGuiInset = true -- IMPORTANT (removes ROBLOX top icon bar gap)
...
image.Size = UDim2.new(1, 0, 1, 0) -- Full screen EXACTLY

image.Position = UDim2.new(0, 0, 0, 0)

image.BackgroundColor3 = Color3.fromRGB(12,12,12)
...
local targetPosition = Vector3.new(-2005.1800537109375, 92.90731811523438, -1774.9578857421875)



-- TELEPORT (SUPER FAST MOVE)
...
addMainMenuButton("AUTO FARM DOMINION BRICK (NXS)", function()
...
addMainMenuButton("PINOY DARK RP (NXS)", function()
...
Frame.BackgroundColor3 = Color3.fromRGB(15,15,15) -- pure black
...
AutoFarmBtn.BackgroundColor3 = Color3.fromRGB(15,15,15)
...
AutoSellBtn.BackgroundColor3 = Color3.fromRGB(15,15,15)
...
addMainMenuButton("BUY GUNSHOP LIST (NXS)", function()
...
addMainMenuButton("CAVITE ROLEPLAY (NXS)", function()
...
addMainMenuButton("BAGONG BARYO SKIN W SOUND (NXS)", function()
...
addMainMenuButton("VENERAL CITY RP (NXS)", function()
...
amountBox.PlaceholderText = "Enter amount (3NXS CMD)"
...
giveOnce.BackgroundColor3 = Color3.fromRGB(15,15,15)

giveOnce.TextColor3 = Color3.fromRGB(255,255,255)
...
addMainMenuButton("BAGONG BARYO LIST (NXS)", function()
...
local function addMainMenuButton(text, callback)
	...
	local panel = allTabPanels["LATEST SCRIPT"]
...
--// Latest Script Button

addMainMenuButton("UNIVERSAL GOD MODE (NXS)", function()

-- INFINITE STATS DOMINION - UNIVERSAL GOD MODE
-- By MAISAN MENU (Universal Edition) - Optimized Anti-Rapid Fire
...
Instance.new("UIStroke", frame).Color = Color3.fromRGB(25, 25, 25)
...
logoBox.Position = UDim2.new(0, 14, 0, 11)
logoBox.BackgroundColor3 = Color3.fromRGB(8, 8, 8)
logoBox.BorderSizePixel = 0
logoBox.Parent = header
Instance.new("UICorner", logoBox).CornerRadius = UDim.new(0, 10)
Instance.new("UIStroke", logoBox).Color = Color3.fromRGB(22, 22, 22)

local logoImage = Instance.new("ImageLabel")
logoImage.Size = UDim2.new(1, -8, 1, -8)
logoImage.Position = UDim2.new(0, 4, 0, 4)
logoImage.BackgroundTransparency = 1
logoImage.Image = "rbxassetid://84114687802252"
logoImage.ScaleType = Enum.ScaleType.Fit
logoImage.Parent = logoBox

local titleLbl = Instance.new("TextLabel")
titleLbl.Size = UDim2.new(0, 160, 0, 26)
titleLbl.Position = UDim2.new(0, 72, 0, 12)
titleLbl.BackgroundTransparency = 1
titleLbl.Text = "UNIVERSAL GOD MODE"
...
subLbl.Size = UDim2.new(0, 160, 0, 18)
subLbl.Position = UDim2.new(0, 73, 0, 38)
subLbl.BackgroundTransparency = 1
subLbl.Text = "Universal Edition"
...
statusBar.Position = UDim2.new(0, 12, 0, 74)
statusBar.BackgroundTransparency = 1
statusBar.Text = "Ready"
statusBar.TextColor3 = Color3.fromRGB(100, 100, 100)
...
divider.BackgroundColor3 = Color3.fromRGB(18, 18, 18)
...
-- TOGGLE CREATOR
-- ============================================
local function createToggle(name, posY, onEnable, onDisable)
    local label = Instance.new("TextLabel")
    label.Size = UDim2.new(0, 150, 0, 26)
    label.Position = UDim2.new(0, 14, 0, posY)
    ...
    toggleBg.Position = UDim2.new(1, -66, 0, posY)
    toggleBg.BackgroundColor3 = Color3.fromRGB(22,22,22)
    toggleBg.BorderSizePixel = 0
    toggleBg.Parent = frame
    Instance.new("UICorner", toggleBg).CornerRadius = UDim.new(0, 13)
    local tbStroke = Instance.new("UIStroke", toggleBg)
    tbStroke.Color = Color3.fromRGB(28, 28, 28)
    ...
    local function toggle()
        ...
        TweenService:Create(knob, TweenInfo.new(0.18, Enum.EasingStyle.Quad), {
            ...
            BackgroundColor3 = enabled and Color3.fromRGB(210,210,210) or Color3.fromRGB(50,50,50)
        }):Play()
        toggleBg.BackgroundColor3 = enabled and Color3.fromRGB(22,22,22) or Color3.fromRGB(10,10,10)
        tbStroke.Color = enabled and Color3.fromRGB(130,130,130) or Color3.fromRGB(28,28,28)
        label.TextColor3 = enabled and Color3.fromRGB(255,255,255) or Color3.fromRGB(160,160,160)
        if enabled then onEnable() else onDisable() end
    end
...
local function refillHealth(h)
    if not h or not h.Parent then return end
    local max = h.MaxHealth
    if max and max > 0 then h.Health = max end
end

local function attachHealthChanged(char)
    local h = char:FindFirstChildOfClass("Humanoid")
    if not h then return end
    if healthChangedConn then healthChangedConn:Disconnect() end
    healthChangedConn = h.HealthChanged:Connect(function(hp)
        if hp < h.MaxHealth then h.Health = h.MaxHealth end
    end)
end

createToggle("Infinite Health", 106, function()
    local char = player.Character
    if char then
        attachHealthChanged(char)
        local h = char:FindFirstChildOfClass("Humanoid")
        if h then refillHealth(h) end
    end
    table.insert(healthConns, player.CharacterAdded:Connect(function(newChar)
        newChar:WaitForChild("Humanoid", 5)
        attachHealthChanged(newChar)
        local h = newChar:FindFirstChildOfClass("Humanoid")
        if h then refillHealth(h) end
    end))
    table.insert(healthConns, RunService.RenderStepped:Connect(function()
        local c = player.Character
        if not c then return end
        local h = c:FindFirstChildOfClass("Humanoid")
        if h and h.Health < h.MaxHealth then h.Health = h.MaxHealth end
    end))
    table.insert(healthConns, RunService.Heartbeat:Connect(function()
        local c = player.Character
        if not c then return end
        local h = c:FindFirstChildOfClass("Humanoid")
        if h then refillHealth(h) end
        pcall(function()
            local rs = game:GetService("ReplicatedStorage")
            local remote = rs:FindFirstChild("Aaron") and
                rs.Aaron:FindFirstChild("Remotes") and
                rs.Aaron.Remotes:FindFirstChild("ModifyStatEvent")
            if remote then remote:InvokeServer("Health", Instance.new("Tool")) end
        end)
    end))
    table.insert(healthConns, RunService.PreSimulation:Connect(function()
        local c = player.Character
        if not c then return end
        local h = c:FindFirstChildOfClass("Humanoid")
        if h then refillHealth(h) end
    end))
    statusBar.Text = "Infinite Health: ON"
    statusBar.TextColor3 = Color3.fromRGB(100, 200, 100)
end, function()
    ...
    statusBar.Text = "Infinite Health: OFF"
    statusBar.TextColor3 = Color3.fromRGB(100, 100, 100)
end)
...
-- ============================================
local armorConn
createToggle("Infinite Armor", 144, function()
    armorConn = RunService.Heartbeat:Connect(function()
        local c = player.Character
        if not c then return end
        for _, v in ipairs(c:GetDescendants()) do
            if v:IsA("NumberValue") or v:IsA("IntValue") then
                local n = v.Name:lower()
                if n == "armor" or n == "shield" or n == "defence" or n == "defense" then
                    v.Value = 999999
                end
            end
        end
        local stats = player:FindFirstChild("leaderstats") or player:FindFirstChild("Stats") or player:FindFirstChild("PlayerStats")
        if stats then
        ...
        pcall(function()
            local rs = game:GetService("ReplicatedStorage")
            local remote = rs:FindFirstChild("Aaron") and rs.Aaron:FindFirstChild("Remotes") and rs.Aaron.Remotes:FindFirstChild("ModifyStatEvent")
            if remote then remote:InvokeServer("Armor", Instance.new("Tool")) end
        end)
    end)
    statusBar.Text = "Infinite Armor: ON"
    statusBar.TextColor3 = Color3.fromRGB(100, 200, 100)
end, function()
    if armorConn then armorConn:Disconnect() armorConn = nil end
    statusBar.Text = "Infinite Armor: OFF"
    statusBar.TextColor3 = Color3.fromRGB(100, 100, 100)
end)
...
-- ============================================
local speedConn
createToggle("Speed Boost", 182, function()
    ...
    statusBar.Text = "Speed Boost: OFF"
    statusBar.TextColor3 = Color3.fromRGB(100, 100, 100)
end)
...
-- ============================================
local jumpConn
createToggle("Infinite Jump", 220, function()
    ...
    statusBar.Text = "Infinite Jump: ON"
    statusBar.TextColor3 = Color3.fromRGB(100, 200, 100)
end, function()
    if jumpConn then jumpConn:Disconnect() jumpConn = nil end
    statusBar.Text = "Infinite Jump: OFF"
    statusBar.TextColor3 = Color3.fromRGB(100, 100, 100)
end)
...
close.BackgroundColor3 = Color3.fromRGB(8, 8, 8)
close.BorderSizePixel = 0
close.Text = "X"
close.TextColor3 = Color3.fromRGB(160, 160, 160)
close.Font = Enum.Font.GothamBold
close.TextSize = 14
close.Parent = frame
Instance.new("UICorner", close).CornerRadius = UDim.new(0, 7)
Instance.new("UIStroke", close).Color = Color3.fromRGB(30, 30, 30)
close.MouseButton1Click:Connect(function()
    for _, c in ipairs(healthConns) do c:Disconnect() end
    if healthChangedConn then healthChangedConn:Disconnect() end
    if armorConn then armorConn:Disconnect() end
...
addMainMenuButton("MANUAL COMSERVE DOMINION WAIT 4 SEC (NXS)", function()
...
-- Enable instant interact for all prompts

for _, prompt in ipairs(workspace:GetDescendants()) do

    if prompt:IsA("ProximityPrompt") then

        prompt.HoldDuration = 0 -- make it instant
...
-- Also catch new prompts that appear later
...
-- Optional: Auto-activate if you press E once near something
...
addMainMenuButton("BAGONG BARYO CONFISCATE GUI (NXS)", function()
...
local SearchRemote = ReplicatedStorage:WaitForChild("Police"):WaitForChild("Search")
...
ScreenGui.Name = "PoliceSearchGUI"
...
ListFrame.Size = UDim2.new(0, 180, 0, 400)

ListFrame.Position = UDim2.new(0, 10, 0.5, -200)
...
local function addPlayerToList(player)
	...
	btn.MouseButton1Click:Connect(function()
		...
		if player.Character then
			...
			SearchRemote:FireServer(player.Character)
...
addMainMenuButton("AFK COMSERV BETA TEST (NXS)", function()
...
_G.WalkEnabled = false

_G.TriggerEnabled = false



-- GUI SETUP — PURE WHITE + BLACK TEXT

local screenGui = Instance.new("ScreenGui")
screenGui.DisplayOrder = 2000
screenGui.ResetOnSpawn = false

screenGui.Name = "WalkTriggerGUI"
...
frame.BackgroundColor3 = Color3.new(1, 1, 1)        -- WHITE NA
...
-- TOGGLE FUNCTION (WHITE THEME)

local function createToggle(name, posY, toggleVar)
	...
	label.TextColor3 = Color3.fromRGB(255,255,255)            -- BLACK TEXT
	...
	toggleFrame.Size = UDim2.new(0, 56, 0, 28)

	toggleFrame.Position = UDim2.new(0, 168, 0, posY - 2)

	toggleFrame.BackgroundColor3 = Color3.fromRGB(230, 230, 230)  -- LIGHT GRAY OFF
	...
	toggleButton.Position = UDim2.new(0, 4, 0.5, -12)

	toggleButton.BackgroundColor3 = Color3.new(1, 1, 1)           -- WHITE KNOB
	...
	toggleFrame.InputBegan:Connect(function(input)
		...
		if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
			...
			_G[toggleVar] = enabled



			if enabled then

				toggleButton.Position = UDim2.new(1, -28, 0.5, -12)

				toggleButton.BackgroundColor3 = Color3.new(1, 1, 1)

				toggleFrame.BackgroundColor3 = Color3.fromRGB(100, 200, 100)  -- GREEN WHEN ON

				label.TextColor3 = Color3.fromRGB(0, 150, 0)
...
createToggle("WALK", 56, "WalkEnabled")

createToggle("TRIGGER", 100, "TriggerEnabled")
...
close.BackgroundColor3 = Color3.new(1, 1, 1)        -- WHITE

close.BorderSizePixel = 1

close.BorderColor3 = Color3.new(0, 0, 0)            -- BLACK BORDER

close.Text = "X"

close.TextColor3 = Color3.fromRGB(255,255,255)                -- BLACK TEXT
...
local folder = workspace:WaitForChild("ComService"):WaitForChild("CleanFolder")
...
local parts = {
	...
	folder:GetChildren()[5]

}



-- WALK LOOP (exact same)
...
task.spawn(function()
	...
	while true do
		...
		if not _G.WalkEnabled then continue end
		...
		for i = 1, #parts do
			...
			if not _G.WalkEnabled then break end
...
task.spawn(function()
	...
	while true do
		...
		if not _G.TriggerEnabled then continue end
		...
		for i = 1, #parts do
			...
			if not _G.TriggerEnabled then break end
...
addMainMenuButton("JOB BOX FARM UNIVERSAL (NXS)", function()
...
moneyLabel.TextColor3 = Color3.fromRGB(0, 170, 0)   -- GREEN PARA KITA PA RIN
...
-- INSTANT Button — WHITE + BLACK TEXT

local instantButton = Instance.new("TextButton")

instantButton.Size = UDim2.new(0, 90, 0, 30)

instantButton.Position = UDim2.new(0, 10, 1, -68)

instantButton.BackgroundColor3 = Color3.new(1, 1, 1)    -- WHITE

instantButton.BorderSizePixel = 1

instantButton.BorderColor3 = Color3.new(0, 0, 0)        -- BLACK BORDER

instantButton.Text = "INSTANT"

instantButton.TextColor3 = Color3.fromRGB(255,255,255)            -- BLACK TEXT
...
-- PICK UP Button — WHITE + BLACK TEXT

local pickUpButton = Instance.new("TextButton")

pickUpButton.Size = UDim2.new(0, 90, 0, 30)

pickUpButton.Position = UDim2.new(1, -100, 1, -68)

pickUpButton.BackgroundColor3 = Color3.new(1, 1, 1)     -- WHITE

pickUpButton.BorderSizePixel = 1

pickUpButton.BorderColor3 = Color3.new(0, 0, 0)         -- BLACK BORDER

pickUpButton.Text = "PICK UP"

pickUpButton.TextColor3 = Color3.fromRGB(255,255,255)             -- BLACK TEXT

pickUpButton.Font = Enum.Font.GothamBold

pickUpButton.TextSize = 14

pickUpButton.Parent = frame
...
pickCorner.Parent = pickUpButton



-- DROP Button — WHITE + BLACK TEXT

local dropButton = Instance.new("TextButton")

dropButton.Size = UDim2.new(0, 210, 0, 28)

dropButton.Position = UDim2.new(0.5, -105, 1, -35)

dropButton.BackgroundColor3 = Color3.new(1, 1, 1)       -- WHITE

dropButton.BorderSizePixel = 1

dropButton.BorderColor3 = Color3.new(0, 0, 0)           -- BLACK BORDER

dropButton.Text = "DROP"

dropButton.TextColor3 = Color3.fromRGB(255,255,255)               -- BLACK TEXT
...
local pickPrompt = workspace.DeliveryJob.BoxPickingJob.PickupBox.PickupPrompt

pickPrompt.MaxActivationDistance = math.huge
...
pickUpButton.MouseButton1Click:Connect(function()
	...
	if autoPick then
		...
		pickUpButton.BackgroundColor3 = Color3.fromRGB(0, 200, 0)  -- GREEN KAPAG ON

		pickUpButton.Text = "PICK UP ON"

		task.spawn(function()

			while autoPick do

				pcall(fireproximityprompt, pickPrompt)
		...
		pickUpButton.BackgroundColor3 = Color3.new(1, 1, 1)
...
local dropPrompt = workspace.DeliveryJob.BoxPickingJob.Job.Part.ProximityPrompt

dropPrompt.MaxActivationDistance = math.huge
...
dropButton.MouseButton1Click:Connect(function()
	...
	if autoDrop then
		...
		dropButton.BackgroundColor3 = Color3.fromRGB(0, 200, 0)    -- GREEN KAPAG ON

		dropButton.Text = "DROP ON"

		task.spawn(function()

			while autoDrop do

				pcall(fireproximityprompt, dropPrompt)
...
instantButton.MouseButton1Click:Connect(function()
	...
	instantButton.BackgroundColor3 = Color3.fromRGB(100, 200, 255)

	task.wait(0.2)

	instantButton.BackgroundColor3 = Color3.new(1, 1, 1)
...
addMainMenuButton("CAR BOOSTER / B TO BOOST / J TO OPEN SETTINGS (NXS)", function()
...
local boostConfig = table.clone(DEFAULTS)
...
local function startBoost(seat)
    ...
    if boosting or not seat then return end
...
local function createSlider(name, min, max, default, onChanged)
    ...
    slider.Size = UDim2.new(1, 0, 0.45, 0)
    ...
    corner.CornerRadius = UDim.new(0.6, 0)
    ...
    fillCorner.CornerRadius = UDim.new(0.6, 0)
    ...
    slider.InputChanged:Connect(function(input)
        ...
        if dragging then updateFromInput(input) end
...
local function createBoostGui()
    ...
    speedLabel.Position = UDim2.new(0, 20, 0.5, -85)
    ...
    local lCorner = Instance.new("UICorner", speedLabel)

    lCorner.CornerRadius = UDim.new(0.2, 0)
    ...
    settingsPanel.Position = UDim2.new(1, -220, 0.5, 0)  -- FIXED RIGHT + CENTERED VERTICALLY
    ...
    UIListLayout.Parent = settingsPanel
    ...
    resetBtn.BackgroundColor3 = Color3.fromRGB(15,15,15)
    ...
    local rCorner = Instance.new("UICorner", resetBtn)

    rCorner.CornerRadius = UDim.new(0.6, 0)



    local sliders = {

        createSlider("Max Force", 50, 1000, boostConfig.MAX_BOOST_FORCE, function(v) boostConfig.MAX_BOOST_FORCE = v end),

        createSlider("Increment", 1, 20, boostConfig.BOOST_INCREMENT, function(v) boostConfig.BOOST_INCREMENT = v end),

        createSlider("Interval", 0.01, 0.5, boostConfig.BOOST_INTERVAL, function(v) boostConfig.BOOST_INTERVAL = v end),

    }

    for i, slider in ipairs(sliders) do

        slider.LayoutOrder = i + 1
    ...
    resetBtn.MouseButton1Click:Connect(function()
        ...
        for _, s in ipairs(sliders) do s:Destroy() end
...
local function setupBoost(character)
    ...
    if playerGui:FindFirstChild("BoostGui") then playerGui.BoostGui:Destroy() end

    local gui, settingsPanel = createBoostGui()
    ...
    UserInputService.InputBegan:Connect(function(input, gameProcessed)
        ...
        if input.KeyCode == Enum.KeyCode.B and currentSeat then

            startBoost(currentSeat)
...
addMainMenuButton("URBAN CITY REVAMPED CONFISCATE GUI (NXS) ", function()
...
local function addPlayerToList(player)
	...
	btn.MouseButton1Click:Connect(function()
		...
		if player.Character then
			...
			ReplicatedStorage:WaitForChild("Police", 9e9):WaitForChild("Search", 9e9):FireServer(unpack(args))
...
addMainMenuButton("DOMINION CITY BANK GUI (NXS)", function()
...
MainFrame.BackgroundColor3 = Color3.new(1, 1, 1)        -- WHITE NA
...
FrameStroke.Color = Color3.new(0, 0, 0)                -- BLACK BORDER
...
TitleBar.BackgroundColor3 = Color3.new(1, 1, 1)        -- WHITE
...
AmountBox.BorderColor3 = Color3.fromRGB(0, 0, 0)       -- BLACK BORDER
...
IDBox.PlaceholderText = "Username"
...
IDBox.BorderColor3 = Color3.fromRGB(0, 0, 0)           -- BLACK BORDER
...
-- Modes Buttons — WHITE + BLACK TEXT
...
for i, mode in ipairs(modes) do
    ...
    btn.BackgroundColor3 = Color3.new(1, 1, 1)         -- WHITE
    ...
    btn.BorderColor3 = Color3.fromRGB(0, 0, 0)         -- BLACK BORDER
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        for _, b in ipairs(modeButtons) do
            ...
            TweenService:Create(b, TweenInfo.new(0.2), {BackgroundColor3 = Color3.new(1, 1, 1)}):Play()
    ...
    if i == 1 then
        ...
        TweenService:Create(btn, TweenInfo.new(0), {BackgroundColor3 = Color3.fromRGB(200, 200, 200)}):Play()
...
-- Execute Button — WHITE + BLACK TEXT
...
ExecuteBtn.BackgroundColor3 = Color3.new(1, 1, 1)       -- WHITE
...
ExecuteBtn.BorderColor3 = Color3.fromRGB(0, 0, 0)       -- BLACK BORDER
...
ExecuteBtn.MouseButton1Click:Connect(function()
    ...
    elseif currentMode == "TRANSFER" then
        ...
        local username = tostring(IDBox.Text)

        if username == "" then return end

        args = { "Transfer", amount, username }
    ...
    game:GetService("ReplicatedStorage")
        ...
        :WaitForChild("Aaron", 9e9)

        :WaitForChild("Shared", 9e9)

        :WaitForChild("Remotes", 9e9)

        :WaitForChild("AtmRemote", 9e9)
...
-- Glow (optional — mas maganda kung grayish white)
...
addMainMenuButton("SILENT AIM BIGHEAD UNIVERSAL (NXS)", function()

   --// CONFIG

_G.HeadSize = 1

_G.HeadTransparency = 1.0
...
local function storeOriginal(player, head)
...
local function updateHead(head, player)
	...
	if not _G.Disabled then
		...
		if data then
			...
			head.BrickColor = data.Color

			head.Material = data.Material

			head.CanCollide = data.Collide
...
RunService.RenderStepped:Connect(function()
	...
	for _, p in pairs(Players:GetPlayers()) do
		...
		if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild("Head") then
			...
			pcall(updateHead, p.Character.Head, p)
...
Frame.BackgroundColor3 = Color3.new(1,1,1)     -- WHITE GUI
...
RunService.RenderStepped:Connect(function()
	...
	local vp = workspace.CurrentCamera.ViewportSize / 2

	Circle.Position = Vector2.new(vp.X, vp.Y)

end)



-- SLIDER LOGIC (NAYARI NA TALAGA, 100% WORKING)
...
local function updateSize(percent)
	...
	_G.HeadSize = 1 + percent * 34

	SizeLabel.Text = "Size: " .. string.format("%.1f", _G.HeadSize)

	Circle.Radius = _G.HeadSize * 5
...
ToggleButton.MouseButton1Click:Connect(function()
	...
	ToggleButton.Text = _G.Disabled and "Disabled" or "Enabled"
...
local FOVToggle = Instance.new("TextButton", Frame)

FOVToggle.Size = UDim2.new(0.8, 0, 0, 25)

FOVToggle.Position = UDim2.new(0.1, 0, 0.83, 0)

FOVToggle.BackgroundColor3 = Color3.fromRGB(12,12,12)

FOVToggle.TextColor3 = Color3.new(1,1,1)

FOVToggle.Text = "FOV: Visible"
...
FOVToggle.MouseButton1Click:Connect(function()
	...
	Circle.Visible = not fovHidden

	FOVToggle.Text = fovHidden and "FOV: Hidden" or "FOV: Visible"
...
addMainMenuButton("BAGONG BARYO AFK COMSERV (NXS)", function()

	-- BYPASS COMSERVE — FULL WHITE THEME + BLACK TEXT ONLY

-- Mas maliit, mas malinis, 100% gumagana pa rin
...
local function setupCamera()
    ...
    cameraConnection = RunService.RenderStepped:Connect(function()
        ...
        if not _G.CameraEnabled then return end

        local flip = math.sign(math.sin(tick() * 25)) * 0.15

        Camera.CFrame = Camera.CFrame * CFrame.new(flip, 0, 0)
...
local function setupInstant()
    ...
    instantConns.added = workspace.DescendantAdded:Connect(function(v)

        if v:IsA("ProximityPrompt") then v.HoldDuration = 0 end

    end)

    instantConns.input = UserInputService.InputBegan:Connect(function(i,gp)

        if gp or i.KeyCode ~= Enum.KeyCode.E then return end
        ...
        for _, p in workspace:GetDescendants() do
            ...
            if p:IsA("ProximityPrompt") and p.Enabled then
                ...
                if (p.Parent.Position - hrp.Position).Magnitude <= p.MaxActivationDistance then
...
local triggerConn, reconnectTask

local function setupTrigger()

    local indexes = {1024,1023,1013,1014,1018,1017,1028,1025,1026,1031,1027,1016}

    local cleaningSpot = workspace:FindFirstChild("CleaningSpot")
    ...
    local function getPrompts()
        ...
        for _, i in indexes do
            ...
            local part = workspace:FindFirstChild(tostring(i))

            if part then

                local p = part:FindFirstChild("ProximityPrompt")
        ...
        if cleaningSpot and cleaningSpot:FindFirstChild("ProximityPrompt") then

            table.insert(t, cleaningSpot.ProximityPrompt)
    ...
    triggerConn = RunService.Heartbeat:Connect(function()
        ...
        for _, p in getPrompts() do fireproximityprompt(p, 0) end
...
local function setupAnimation()
    ...
    local function onChar(char)
        ...
        for _, sec in {"idle","fall","climb","toolnone","sit","freefall"} do

            local s = animate:FindFirstChild(sec)

            if s then for _, a in s:GetChildren() do if a:IsA("StringValue") then a.AnimationId = "" end end end

        end

        local conn = RunService.Heartbeat:Connect(function()

            if not _G.AnimationEnabled then return end

            for _, track in hum:GetPlayingAnimationTracks() do

                local id = track.Animation.AnimationId:lower()

                if not (id:find("walk") or id:find("run") or id:find("swim")) and track.Priority ~= Enum.AnimationPriority.Movement then
    ...
    animConns.added = player.CharacterAdded:Connect(onChar)

end



-- GUI — PURE WHITE + BLACK TEXT
...
-- Toggle Creator (White + Black Text)

local function createToggle(name, y, var, on, off)

    local lbl = Instance.new("TextLabel")

    lbl.Size = UDim2.new(0,140,0,36)
    ...
    bg.Size = UDim2.new(0,62,0,32)

    bg.Position = UDim2.new(1,-82,0,y+2)
    ...
    bg.InputBegan:Connect(function(i)
        ...
        if _G[var] then
            ...
            knob.Position = UDim2.new(1,-32,0.5,-14)
...
createToggle("INSTANT INTERACT", 60, "InstantEnabled", setupInstant, function()

    for _,c in instantConns do if c.Connected then c:Disconnect() end end
...
createToggle("TELEPORT", 100, "TeleportEnabled", function()

    local pos = CFrame.new(503.67, 4.95, -765.29)
...
createToggle("AUTO TRIGGER", 140, "TriggerEnabled", setupTrigger, function()
...
createToggle("NO ANIMATION", 180, "AnimationEnabled", setupAnimation, function()

    for _,c in animConns do if c.Connected then c:Disconnect() end end
...
createToggle("CAMERA SHAKE", 220, "CameraEnabled", setupCamera, function()
...
close.MouseButton1Click:Connect(function()
    ...
    _G.InstantEnabled = false _G.TeleportEnabled = false

    _G.TriggerEnabled = false _G.AnimationEnabled = false _G.CameraEnabled = false
...
local function addMainMenuButton(text, callback)
	...
	local panel = allTabPanels["BRING MENU"]
...
addMainMenuButton("CARRY SYSTEM BRING ALL 1 (NXS)", function()
...
commands["Secret"] = function()
	...
	chatService:FireServer("NEXUS_INTERNALMANTOT", "All")
...
addMainMenuButton("CARRY SYSTEM BRING ALL 2 (NXS)", function()
...
addMainMenuButton("CARRY SYSTEM BRING ALL (VERSION 3)", function()
...
local function addMainMenuButton(text, callback)
	...
	local panel = allTabPanels["AIMBOT MENU"]
...
addMainMenuButton("ALADIA AIMBOT VERSION 2 (NXS)", function()
...
addMainMenuButton("FANTASMA AIMBOT (DETECTED)", function()
...
addMainMenuButton("AIMBOT ON AND OFF (CHESTER)", function()
...
addMainMenuButton("AIMBOT SUPPORTED CELLPHONE (NXS)", function()
...
local function addMainMenuButton(text, callback)
	...
	local panel = allTabPanels["SCRIPT MENU"]
...
addMainMenuButton("3KTG BACKDOOR SCANNER (NXS)", function()
...
addMainMenuButton("BRING PART (NXS)", function()
...
addMainMenuButton("CLIENT SPAWNER (NXS)", function()
...
addMainMenuButton("ADMIN TROLL (NXS)", function()
...
addMainMenuButton("FLING SELECT (NXS)", function()
...
addMainMenuButton("JERK (NXS)", function()
...
addMainMenuButton("CHAT TROLL (NXS)", function()
...
addMainMenuButton("SHY GUY (NXS)", function()
...
addMainMenuButton("AIMBOT V2 (NXS)", function()
...
addMainMenuButton("V1 TROLL (NXS)", function()
...
addMainMenuButton("V2 TROLL (NXS)", function()
...
addMainMenuButton("ADONIS BYPASS CP (NXS)", function()
...
addMainMenuButton("CLIENT BRING  (NXS)", function()
...
addMainMenuButton("", function()

    -- LocalScript (put in StarterPlayerScripts)

-- SPAM SOUND GUI with toggle (draggable, black, rounded corners)
...
-- UPDATED REMOTE (Your new system)

local dropRemote = ReplicatedStorage:WaitForChild("DropRequest", 9e9)
...
toggleBtn.Position = UDim2.new(0.5, -60, 0, 46)
...
statusDot.BackgroundColor3 = Color3.fromRGB(170,170,170)
...
addMainMenuButton("", function()
   ...
   -- LocalScript (put inside StarterPlayerScripts)
...
local comboRemote = ReplicatedStorage:WaitForChild("GiveCombo3", 9e9)
...
frame.Position = UDim2.new(0.5, -115, 0.5, -55)
	...
	thread = task.spawn(function()
		...
		while isOn do
			...
			pcall(function()
				...
				comboRemote:FireServer(unpack(args))
...
local function addMainMenuButton(text, callback)
	...
	local panel = allTabPanels["CASH MENU"]
...
addMainMenuButton("KALYE ONSE (NXS)", function()
...
addMainMenuButton("TONDO ROLEPLAY (NXS)", function()
...
addMainMenuButton("THE BILLIONARE CITY (NXS)", function()

    -- LocalScript: MONEY GUI – Pure White + Black Text Only
...
moneyLabel.TextColor3 = Color3.fromRGB(255,255,255)  -- Black na rin (walang green)
...
-- CASH Button (White bg, black text)
...
cashButton.BackgroundColor3 = Color3.new(1,1,1)  -- White

cashButton.Text = "CASH"

cashButton.TextColor3 = Color3.fromRGB(255,255,255)  -- Black text
...
toggleFrame.InputBegan:Connect(function(input)
	...
	if input.UserInputType == Enum.UserInputType.MouseButton1 then
		...
		if isOn then
			...
			toggleFrame.BackgroundColor3 = Color3.fromRGB(20,20,20)  -- Light gray when ON
...
addMainMenuButton("PROMINENCE CITY S1 (NXS)", function()
...
addMainMenuButton("URBAN CITY V3 (NXS)", function()
...
sendButton.MouseButton1Click:Connect(function()
    ...
    if amount then
        ...
        game:GetService("ReplicatedStorage").UwUCafeFolder.BuyTool:FireServer("Prime", -amount)
...
local function addMainMenuButton(text, callback)
	...
	local panel = allTabPanels["PLAYER MENU"]
...
addMainMenuButton("USERNAME COPIER (NXS)", function()
...
addMainMenuButton("TRACK ALL MUSCLE T TO OPEN / OFF (NXS)", function()
...
warn("🔁 Press [T] to toggle Player Tracker with Clean ESP.")

end)



addMainMenuButton("ESP (NXS)", function()
...
local function addMainMenuButton(text, callback)
	...
	local panel = allTabPanels["NEXUS V4"]
...
addMainMenuButton("FPS BOOST CARTOON (NXS)", function()
...
Button.BackgroundColor3 = Color3.fromRGB(15,15,15) -- black button
...
addMainMenuButton("LASPINAS SYSTEM (NXS)", function()
...
title.Text = "NXS VENERAL ROLEPLAY"
...
addMainMenuButton("ADJUSTABLE BIG HEAD (NXS)", function()
...
ToggleButton.BackgroundColor3 = Color3.fromRGB(15,15,15) -- Black
...
addMainMenuButton("PERSIONAL SCRIPT FOR (NXS)", function()
...
SliderBack.Position = UDim2.new(0.1, 0, 0.27, 0)
...
SliderButton.MouseButton1Down:Connect(function() draggingSize = true end)

UIS.InputEnded:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 then draggingSize = false end end)

UIS.InputChanged:Connect(function(input)

	if draggingSize and input.UserInputType == Enum.UserInputType.MouseMovement then

		updateSize((UIS:GetMouseLocation().X - SliderBack.AbsolutePosition.X) / SliderBack.AbsoluteSize.X)
...
-- TRANSPARENCY SLIDER (WHITE)

-------------------------------------------------

local TBack = Instance.new("Frame", Frame)

TBack.Size = UDim2.new(0.8, 0, 0, 6)

TBack.Position = UDim2.new(0.1, 0, 0.55, 0)

TBack.BackgroundColor3 = Color3.fromRGB(12,12,12)

round(TBack, 8)



local TFill = Instance.new("Frame", TBack)

TFill.Size = UDim2.new(_G.HeadTransparency, 0, 1, 0)

TFill.BackgroundColor3 = Color3.new(1,1,1)

round(TFill, 8)



local TButton = Instance.new("TextButton", TBack)

TButton.Size = UDim2.new(0, 20, 0, 20)

TButton.Position = UDim2.new(_G.HeadTransparency, -10, 0.5, -10)

TButton.BackgroundColor3 = Color3.new(1,1,1)
...
TLabel.Position = UDim2.new(0, 0, 0.62, 0)

TLabel.BackgroundTransparency = 1

TLabel.TextColor3 = Color3.fromRGB(255,255,255)

TLabel.Text = "Transparency: " .. string.format("%.2f", _G.HeadTransparency)
...
local function updateTransparency(percent)
	...
	TFill.Size = UDim2.new(percent, 0, 1, 0)

	TButton.Position = UDim2.new(percent, -10, 0.5, -10)

	_G.HeadTransparency = percent

	TLabel.Text = "Transparency: " .. string.format("%.2f", percent)

end



TButton.MouseButton1Down:Connect(function() draggingT = true end)

UIS.InputEnded:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 then draggingT = false end end)

UIS.InputChanged:Connect(function(input)

	if draggingT and input.UserInputType == Enum.UserInputType.MouseMovement then

		updateTransparency((UIS:GetMouseLocation().X - TBack.AbsolutePosition.X) / TBack.AbsoluteSize.X)
...
ToggleButton.MouseButton1Click:Connect(function()
	...
	ToggleButton.Text = _G.Disabled and "Disabled: On" or "Disabled: Off"
...
updateTransparency(_G.HeadTransparency)
...
addMainMenuButton("PERSONAL SCRIPT FOR (VASCO)", function()
...
HeadToggleButton.BackgroundColor3 = Color3.fromRGB(15,15,15)
...
AimToggleButton.BackgroundColor3 = Color3.fromRGB(15,15,15)
...
addMainMenuButton("FREECAM TELEPORT PRESS Q (NXS)", function()
...
local lastCamCFrame -- ✅ store final freecam position
...
local function toggleFreecam()
	...
	if enabled then
		...
		-- ✅ Save final camera position before restoring
		...
		-- ✅ Teleport character where freecam stopped
...
addMainMenuButton("CASH DROP (NXS)", function()

	-- NXS CASH DROP — FULL WHITE THEME + BLACK TEXT

-- Mas maliit, mas malinis, zero dark elements
...
frame.Size = UDim2.new(0, 310, 0, 195)

frame.Position = UDim2.new(0.5, -155, 0.5, -97)
...
-- Username Box (White bg na rin!)

local userBox = Instance.new("TextBox")

userBox.Size = UDim2.new(1,-40,0,40)

userBox.Position = UDim2.new(0,20,0,55)

userBox.BackgroundColor3 = Color3.new(1,1,1)        -- white bg
...
local pad1 = Instance.new("UIPadding", userBox)

pad1.PaddingLeft = UDim.new(0,14)



-- Amount Box (White bg na rin!)

local amountBox = Instance.new("TextBox")

amountBox.Size = UDim2.new(1,-40,0,40)

amountBox.Position = UDim2.new(0,20,0,105)

amountBox.BackgroundColor3 = Color3.new(1,1,1)   -- white bg

amountBox.PlaceholderText = "Enter amount (ex: 0.1)"
...
local pad2 = Instance.new("UIPadding", amountBox)

pad2.PaddingLeft = UDim.new(0,14)

pad2.Parent = amountBox
...
startBtn.Size = UDim2.new(0.48,0,0,44)

startBtn.Position = UDim2.new(0,20,1,-20)
...
stopBtn.Size = UDim2.new(0.48,0,0,44)

stopBtn.Position = UDim2.new(0.52,0,1,-20)

stopBtn.AnchorPoint = Vector2.new(0,1)

stopBtn.BackgroundColor3 = Color3.new(1,1,1)
...
startBtn.MouseButton1Click:Connect(function()
    ...
    status.Text = "Status: Dropping cash..."

    status.TextColor3 = Color3.fromRGB(0,120,0)
    ...
    task.spawn(function()
        ...
        while running do
            ...
            local amt = tonumber(amountBox.Text) or 0.1

            ReplicatedStorage.DropCash:FireServer(player.Name, amt)
...
addMainMenuButton("PERSONAL SCRIPT FOR (CHESTER)", function()
...
title.Text = "CROSSHAIR AIMBOT - NXS"
...
modeDropdown.BackgroundColor3 = Color3.fromRGB(15,15,15)
...
smoothBox.BackgroundColor3 = Color3.fromRGB(15,15,15)
...
addMainMenuButton("BAGONG BARYO CITY (NXS)", function()
...
addMainMenuButton("BAGONG BARYO SPAWN GUN (SANTINO)", function()
...
local function addMainMenuButton(text, callback)
	...
	local panel = allTabPanels["THE BILLIONARE"]
...
addMainMenuButton("THE BILLIONARE CITY SPAWN GUN (NXS)", function()
...
title.Text = "THE BILLIONAIRE CITY RP"
...
dropdownBtn.BackgroundColor3 = Color3.new(1,1,1)
...
local padding = Instance.new("UIPadding", dropdownBtn)
...
dropList.Size = UDim2.new(1, -40, 0, 180)

dropList.Position = UDim2.new(0, 20, 0, 130)
...
local items = {
    ...
    ".50 Pink", "Balistic Knife", "Blueprint", "Combat PD",

    "Hoverboard", "LockPick", "PD Cuff", "Rock",

    "PistolBody", "SharkFang", "SharkBone", "TASER",

    "Stone", "Oxy Pill", "SwitchBlade"
...
dropList.CanvasSize = UDim2.new(0, 0, 0, #items * 46)
...
autoBuy.Size = UDim2.new(1, -40, 0, 55)

autoBuy.Position = UDim2.new(0, 20, 1, -20)

autoBuy.AnchorPoint = Vector2.new(0, 1)

autoBuy.BackgroundColor3 = Color3.new(1,1,1)

autoBuy.Text = "START AUTO BUY"
...
Instance.new("UICorner", autoBuy).CornerRadius = UDim.new(0, 14)
...
autoBuy.MouseButton1Click:Connect(function()
    ...
    autoBuy.Text = running and "STOP AUTO BUY" or "START AUTO BUY"



    if running and selectedItem then

        task.spawn(function()

            while running do

                ReplicatedStorage:WaitForChild("BuyToolEvent"):FireServer(selectedItem, 1, "Kaizeuuu")
...
addMainMenuButton("THE BILLIONARE CITY SPAWN CASH (NXS)", function()
...
addMainMenuButton("THE BILLIONARE CITY REFRESH (NXS)", function()
...
addMainMenuButton("THE BILLIONARE CITY SPAWN GUI (NXS)", function()

	-- LocalScript: NXS — Buy Tool (Super Compact White GUI)
...
-- Main Frame (mas maliit na)

local frame = Instance.new("Frame")

frame.Size = UDim2.new(0, 280, 0, 160)           -- mas compact
...
shadow.Size = UDim2.new(1, 10, 1, 16)

shadow.Position = UDim2.new(0, -5, 0, 8)

shadow.BackgroundTransparency = 0.85

shadow.BackgroundColor3 = Color3.fromRGB(12,12,12)
...
-- TextBox (mas maliit at mas maganda)

local box = Instance.new("TextBox")

box.Size = UDim2.new(1, -32, 0, 38)

box.Position = UDim2.new(0, 16, 0, 46)

box.BackgroundColor3 = Color3.fromRGB(248, 248, 250)

box.PlaceholderText = "Tool name here..."
...
-- BUY Button (white na rin, black text)

local btn = Instance.new("TextButton")

btn.Size = UDim2.new(1, -32, 0, 42)

btn.Position = UDim2.new(0, 16, 1, -58)
...
-- Hover effect (light gray lang para subtle)

btn.MouseEnter:Connect(function()

	TweenService:Create(btn, TweenInfo.new(0.18), {BackgroundColor3 = Color3.fromRGB(240, 240, 245)}):Play()

end)

btn.MouseLeave:Connect(function()

	TweenService:Create(btn, TweenInfo.new(0.18), {BackgroundColor3 = Color3.new(1,1,1)}):Play()

end)



-- Feedback (very small sa baba)

local feedback = Instance.new("TextLabel")

feedback.Size = UDim2.new(1, 0, 0, 20)

feedback.Position = UDim2.new(0, 0, 1, -24)

feedback.BackgroundTransparency = 1

feedback.Text = ""

feedback.TextColor3 = Color3.fromRGB(0, 120, 0)
...
local function show(msg)
	...
	task.delay(2, function()
		...
		if feedback.Parent then feedback.Text = "" end
...
local function buy()
	...
	local name = box.Text:match("^%s*(.-)%s*$")  -- trim

	if name == "" then

		show("Enter tool name!")
	...
	buyRemote:FireServer(name, 1, "Kaizeuuu")
...
btn.MouseButton1Click:Connect(buy)



-- Enter key sa textbox = buy din

box.FocusLost:Connect(function(enter)

	if enter then buy() end
...
player.AncestryChanged:Connect(function()
	...
	if not player.Parent then gui:Destroy() end
...
addMainMenuButton("THE BILLIONARE CITY SPAM SOUND (NXS)", function()
...
-- Remote reference (uses user's provided pattern)

local vaultRemote = ReplicatedStorage:WaitForChild("Vault", 9e9)
...
main.BackgroundColor3 = Color3.fromRGB(15,15,15) -- all black
...
do
	...
	main.InputBegan:Connect(inputBegan)

	main.InputChanged:Connect(inputChanged)
...
local function startSpam()
	...
	spamTask = task.spawn(function()
		...
		-- local args table as user provided
		...
		while isOn do
			...
			local ok, err = pcall(function()
				...
				vaultRemote:FireServer(unpack(args))

			end)

			-- silently ignore errors so the loop continues but avoid tight infinite error floods

			if not ok then

				-- optional: you can print(err) for debugging

				-- print("FireServer error:", err)

			end

			-- smallest reasonable wait; task.wait(0.001) will wait ~1ms but can be limited by scheduler
...
local function stopSpam()
	...
	-- spamTask will end by itself because loop checks isOn
...
-- Optional: allow closing with right-click on title (not required)
...
addMainMenuButton("THE BILLIONAIRE CITY STORAGE SYSTEM", function()

    -- LocalScript: Tools Viewer (White GUI + Black Text + Light Accents)
...
-- Soft shadow effect (optional but nice on white)

local shadow = Instance.new("Frame")

shadow.Size = UDim2.new(1, 8, 1, 20)

shadow.Position = UDim2.new(0, -4, 0, 10)
...
subtitle.Text = "Click any tool to copy its name"
...
-- ScrollingFrame (slightly off-white for depth)
...
scrolling.BackgroundColor3 = Color3.fromRGB(248, 248, 250)

scrolling.BorderSizePixel = 0

scrolling.ScrollBarThickness = 8

scrolling.ScrollBarImageColor3 = Color3.fromRGB(180, 180, 190)
...
local function addTool(tool)
	...
	btn.MouseEnter:Connect(function()
		...
		TweenService:Create(btn, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(235, 245, 255)}):Play()
	...
	btn.MouseButton1Click:Connect(function()
		...
		btn.TextColor3 = Color3.fromRGB(0, 140, 0)

		btn.Text = "  " .. tool.Name .. "  [Copied!]"
...
local function rebuildList()
	...
	scrolling.CanvasSize = UDim2.new(0, 0, 0, #list * 42)

end



-- Initial build + live updates
...
local function addMainMenuButton(text, callback)
	...
	local panel = allTabPanels["RP TELEPORT"]
...
local function addMainMenuButton(text, callback)
	...
	local panel = allTabPanels["ADMIN PANEL"]
...
addMainMenuButton("NOTE : ONLY WHITLISTED USER CAN USE THIS !", function()
...
addMainMenuButton("ADMIN PANEL (NXS)", function()

local ok, err = pcall(function()
    local player = game:GetService("Players").LocalPlayer
    local panel = player.PlayerGui:WaitForChild("ExtraUI"):WaitForChild("AdminPanel")
    panel.Transparency = 0.7
    panel.Visible = true
end)
if not ok then
    warn("Admin Panel not found in this game!")
end
...
addMainMenuButton("SET SKYBOX (NXS)", function()
...
if not tool then warn("Kailangan ng BTools!") return end
local remote = tool.SyncAPI.ServerEndpoint
local function _(args) remote:InvokeServer(unpack(args)) end
local function CreatePart(cf,parent) _({[1]="CreatePart",[2]="Normal",[3]=cf,[4]=parent}) end
local function SetName(part,s) _({[1]="SetName",[2]={[1]=part},[3]=s}) end
local function AddMesh(part) _({[1]="CreateMeshes",[2]={[1]={["Part"]=part}}}) end
local function SetMesh(part,id) _({[1]="SyncMesh",[2]={[1]={["Part"]=part,["MeshId"]="rbxassetid://"..id}}}) end
local function SetTexture(part,id) _({[1]="SyncMesh",[2]={[1]={["Part"]=part,["TextureId"]="rbxassetid://"..id}}}) end
local function MeshResize(part,size) _({[1]="SyncMesh",[2]={[1]={["Part"]=part,["Scale"]=size}}}) end
local function SetLocked(part,bool) _({[1]="SetLocked",[2]={[1]=part},[3]=bool}) end
...
addMainMenuButton("KILL ALL  ", function()
...
for _, targetPlayer in ipairs(Players:GetPlayers()) do
    ...
    player
        ...
        :WaitForChild("ExtraUI")

        :WaitForChild("AdminPanel")
...
local function addMainMenuButton(text, callback)
	...
	local panel = allTabPanels["REMOTES"]
...
addMainMenuButton("REMOTES ONLY FOR NEXUS ! ", function()

	-- NEXUS REMOTE SPY v3 - WITH USERNAME WHITELIST

-- Copy-paste this into your executor
...
leftPanel.Size = UDim2.new(0, 200, 0, 330)
...
rightPanel.Size = UDim2.new(0, 450, 0, 330)

rightPanel.Position = UDim2.new(0, 235, 0, 50)
...
local logScroll = Instance.new("ScrollingFrame", rightPanel)
...
local btnContainer = Instance.new("Frame", rightPanel)

btnContainer.Size = UDim2.new(1, -10, 0, 35)

btnContainer.Position = UDim2.new(0.5, -175, 1, -40)
...
local fireBtn = makeBtn("FIRE", 0)

local startBtn = makeBtn("START", 90)

local clearBtn = makeBtn("CLEAR", 180)

local copyBtn = makeBtn("COPY", 270)
...
local insertionQueue = insertionQueue or {}
...
local function getRemotes()
    ...
    for _, v in pairs({game.ReplicatedStorage, game.Workspace, game.StarterGui, game.StarterPack, game.StarterPlayer.StarterPlayerScripts}) do

        if v then scan(v) end
...
local function createButton(remote)
    ...
    buttonMap[remote] = btn



    btn.MouseButton1Click:Connect(function()

        for _, b in pairs(leftScroll:GetChildren()) do
...
local function autoInsertToQueue(remote, argsTable)

    local log = "Remote Fired: " .. remote:GetFullName()

    local argsStr = "{"

    for i, arg in ipairs(argsTable) do

        argsStr = argsStr .. (i > 1 and ", " or "") .. tostr(arg)
    ...
    local scriptLine = "local args = " .. argsStr .. "\n\n" .. tostr(remote) .. ":FireServer(unpack(args))"



    table.insert(insertionQueue, { log, "To", scriptLine, nil, remote, argsTable, "FireServer" })



    local btn = buttonMap[remote]
...
local function hookRemote(remote)
    ...
    if remote:IsA("RemoteEvent") then
        ...
        remote.FireServer = function(self, ...)
            ...
            task.spawn(autoInsertToQueue, remote, args)
...
leftScroll.CanvasSize = UDim2.new(0, 0, 0, #remotes * 28)
...
fireBtn.MouseButton1Click:Connect(function()
    ...
    if not selectedRemote then return addLog("Select remote first!") end

    autoInsertToQueue(selectedRemote, {})
...
startBtn.MouseButton1Click:Connect(function()
    ...
    startBtn.Text = autoFiring and "RUNNING" or "START"

    addLog(autoFiring and "Auto firing started." or "Stopped auto firing.")
...
copyBtn.MouseButton1Click:Connect(function()
    ...
    local scriptLine = "local args = {}\n\n" .. tostr(selectedRemote) .. ":FireServer(unpack(args))"

    if setclipboard then

        setclipboard(scriptLine)
...
addLog("✅ NEXUS REMOTE SPY LOADED (WHITELIST MODE)")
...
-- PLAYER LIST – LIGHT GRAY MINI BUTTONS
		...
		if plr ~= player then
			...
			local function makeMiniBtn(text, callback)
				...
				local b = new("TextButton", {
					...
					BackgroundColor3 = Color3.fromRGB(12,12,12), -- light gray button
...
local transLabel = new("TextLabel", {

	Parent = settingsPanel,

	Size = UDim2.new(1, 0, 0, 30),

	Text = "GUI Transparency: 0%",
...
new("UICorner", {CornerRadius = UDim.new(0,10), Parent = slider})
...
slider.InputBegan:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 then draggingSlider = true end end)
...
UserInputService.InputChanged:Connect(function(i)
	...
	if draggingSlider and i.UserInputType == Enum.UserInputType.MouseMovement then
		...
		main.BackgroundTransparency = p * 0.7

		transLabel.Text = "GUI Transparency: " .. math.floor(p * 100) .. "%"
...
new("UICorner", {CornerRadius = UDim.new(0,6), Parent = keybindBtn})
...
UserInputService.InputBegan:Connect(function(i)
	...
	if listening and i.KeyCode ~= Enum.KeyCode.Unknown then
		...
		keybindLabel.Text = "Toggle Key: " .. i.KeyCode.Name

		keybindBtn.Text = "Change Toggle Key"
...
-- LUA EXECUTOR – LIGHT THEME TEXTBOX
...
new("UICorner", {CornerRadius = UDim.new(0,6), Parent = execBtn})
...
do
	...
	main.InputBegan:Connect(function(i)
		...
		if i.UserInputType == Enum.UserInputType.MouseButton1 then
			...
			draggingFrame = true; dragStart = i.Position; startPos = main.Position

			i.Changed:Connect(function() if i.UserInputState == Enum.UserInputState.End then draggingFrame = false end end)
	...
	UserInputService.InputChanged:Connect(function(i)
		...
		if i == dragInput and draggingFrame then update(i) end
...
UserInputService.InputBegan:Connect(function(i, gp)
	...
	if i.KeyCode == toggleKey then
		...
		if screenGui.Enabled then
			...
			for _, b in ipairs(allTabButtons) do b.BackgroundColor3 = Color3.fromRGB(8,8,8) end

			if allTabButtons[1] then allTabButtons[1].BackgroundColor3 = Color3.fromRGB(25,25,25) end
...
-- ============================================================
-- NEW SCRIPTS FROM MONSY MENU V4 (added to NEXUS V4)
-- Source: MONSY_MENU_V4_OPEN_SRC by @kvna / sevic
-- ============================================================

addMainMenuButton("ANTI SUSPEND (NXS)", function()
...
VoiceChatService =
    ...
    CoreGui =
    task.spawn(function()
        pcall(function(a_734, b_734, c_734)
            GroupId = VoiceChatInternal:GetGroupId();
    ...
    task.spawn(function()
        MonsyNotif = Instance.new"ScreenGui";
        MonsyNotif.Name = "MonsyNotif";
        MonsyNotif.IgnoreGuiInset = true;
        MonsyNotif.ResetOnSpawn = false;
        MonsyNotif.Parent = gethui();
        Frame_7 = Instance.new"Frame";
        Frame_7.Size = UDim2.new(0, 300, 0, 70);
        Frame_7.Position = UDim2.new(1, 10, 1, -90);
        Frame_7.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
        Frame_7.BorderSizePixel = 0;
        Frame_7.Parent = MonsyNotif;
        Instance.new("UICorner", Frame_7).CornerRadius = UDim_New(0, 14);
        TextLabel_4 = Instance.new"TextLabel";
        TextLabel_4.Size = UDim2.new(0, 40, 1, 0);
        TextLabel_4.Position = UDim2.new(0, 10, 0, 0);
        TextLabel_4.BackgroundTransparency = 1;
        TextLabel_4.Text = "✔";
        TextLabel_4.TextColor3 = Color3.fromRGB(255, 255, 255);
        TextLabel_4.Font = CoreGui.GothamBold;
        TextLabel_4.TextSize = 22;
        TextLabel_4.TextXAlignment = CoreGui.Center;
        TextLabel_4.Parent = Frame_7;
        MONSY_ANTI_SUSPEND = Instance.new"TextLabel";
        MONSY_ANTI_SUSPEND.Size = UDim2.new(1, -65, 0, 28);
        MONSY_ANTI_SUSPEND.Position = UDim2.new(0, 58, 0, 10);
        MONSY_ANTI_SUSPEND.BackgroundTransparency = 1;
        MONSY_ANTI_SUSPEND.Text = "MONSY ANTI SUSPEND";
        MONSY_ANTI_SUSPEND.TextColor3 = Color3.fromRGB(255, 255, 255);
        MONSY_ANTI_SUSPEND.Font = CoreGui.GothamBold;
        MONSY_ANTI_SUSPEND.TextSize = 14;
        MONSY_ANTI_SUSPEND.TextXAlignment = CoreGui.Left;
        MONSY_ANTI_SUSPEND.Parent = Frame_7;
        TextLabel_5 = Instance.new"TextLabel";
        TextLabel_5.Size = UDim2.new(1, -65, 0, 24);
        TextLabel_5.Position = UDim2.new(0, 58, 0, 34);
        TextLabel_5.BackgroundTransparency = 1;
        TextLabel_5.Text = "Anti Vc Suspend Activated\nSuccessfully!";
        TextLabel_5.TextColor3 = Color3.fromRGB(255, 255, 255);
        TextLabel_5.Font = CoreGui.Gotham;
        TextLabel_5.TextSize = 12;
        TextLabel_5.TextXAlignment = CoreGui.Left;
        TextLabel_5.TextWrapped = true;
        TextLabel_5.Parent = Frame_7;
        Frame_7:TweenPosition(UDim2.new(1, -315, 1, -90), "Out", "Back", 0.5, true);
        task.wait(4);
        Frame_7:TweenPosition(UDim2.new(1, 10, 1, -90), "In", "Quad", 0.5, true);
        task.wait(0.5);
        MonsyNotif:Destroy();
...
addMainMenuButton("ANTI SUSPEND FEARLESS (NXS)", function()
...
addMainMenuButton("CITY OF LAGUNA SPAWN GUN (NXS)", function()

ScreenGui_58 = Instance.new"ScreenGui";
    ScreenGui_58.Name = "";
    ScreenGui_58.ResetOnSpawn = false;
    ScreenGui_58.Parent = game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui");
    Frame_134 = Instance.new"Frame";
    Frame_134.Position = UDim2.new(0.5, -130, 0.5, -160);
    Frame_134.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    Frame_134.BorderSizePixel = 1;
    Frame_134.BorderColor3 = Color3.fromRGB(40, 40, 40);
    Frame_134.Parent = ScreenGui_58;
    Frame_134.Active = true;
    Frame_134.Draggable = true;
    UICorner_323 = Instance.new"UICorner";
    UICorner_323.CornerRadius = UDim_New(0, 10);
    UICorner_323.Parent = Frame_134;
    CITY_OF_LAGUNA = Instance.new"TextLabel";
    CITY_OF_LAGUNA.Size = UDim2.new(1, 0, 0, 40);
    CITY_OF_LAGUNA.Position = UDim2.new(0, 0, 0, 0);
    CITY_OF_LAGUNA.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    CITY_OF_LAGUNA.Text = "CITY OF LAGUNA";
    CITY_OF_LAGUNA.TextColor3 = Color3.fromRGB(255, 255, 255);
    CITY_OF_LAGUNA.Font = Enum.Font.Roboto;
    CITY_OF_LAGUNA.TextSize = 16;
    CITY_OF_LAGUNA.Parent = Frame_134;
    Instance.new("UICorner", CITY_OF_LAGUNA).CornerRadius = UDim_New(0, 8);
    Select_Item_10 = Instance.new"TextButton";
    Select_Item_10.Size = UDim2.new(1, -20, 0, 30);
    Select_Item_10.Position = UDim2.new(0, 10, 0, 50);
    Select_Item_10.Text = "Select Item";
    Select_Item_10.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    Select_Item_10.TextColor3 = Color3.fromRGB(255, 255, 255);
    Select_Item_10.Font = Enum.Font.Roboto;
    Select_Item_10.TextSize = 14;
    Select_Item_10.Parent = Frame_134;
    Instance.new("UICorner", Select_Item_10).CornerRadius = UDim_New(0, 6);
    Frame_135 = Instance.new"Frame";
    Frame_135.Size = UDim2.new(1, -20, 0, 56);
    Frame_135.Position = UDim2.new(0, 10, 0, 85);
    Frame_135.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    Frame_135.Visible = false;
    Frame_135.Parent = Frame_134;
    Instance.new("UICorner", Frame_135).CornerRadius = UDim_New(0, 5);
    Silencer_Pistolz = Instance.new"TextButton";
    Silencer_Pistolz.Size = UDim2.new(1, 0, 0, 28);
    Silencer_Pistolz.Position = UDim2.new(0, 0, 0, 0);
    Silencer_Pistolz.Text = "Silencer Pistolz";
    Silencer_Pistolz.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    Silencer_Pistolz.TextColor3 = Color3.fromRGB(255, 255, 255);
    Silencer_Pistolz.Font = Enum.Font.Roboto;
    Silencer_Pistolz.TextSize = 12;
    Silencer_Pistolz.Parent = Frame_135;
    Instance.new("UICorner", Silencer_Pistolz).CornerRadius = UDim_New(0, 5);
    Silencer_Pistolz.MouseButton1Click:Connect(function(a_1112, b_1112, c_1112)
        Select_Item_10.Text = "Silencer Pistolz";
        Frame_135.Visible = false;
    end);
    Monsy_3 = Instance.new"TextButton";
    Monsy_3.Size = UDim2.new(1, 0, 0, 28);
    Monsy_3.Position = UDim2.new(0, 0, 0, 28);
    Monsy_3.Text = "// @ Monsy";
    Monsy_3.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    Monsy_3.TextColor3 = Color3.fromRGB(255, 255, 255);
    Monsy_3.Font = Enum.Font.Roboto;
    Monsy_3.TextSize = 12;
    Monsy_3.Parent = Frame_135;
    Instance.new("UICorner", Monsy_3).CornerRadius = UDim_New(0, 5);
    Monsy_3.MouseButton1Click:Connect(function(a_1113, b_1113, c_1113)
        Select_Item_10.Text = "// @ Monsy";
        Frame_135.Visible = false;
    end);
    Select_Item_10.MouseButton1Click:Connect(function(a_1114, b_1114, c_1114)
        local _ = not Frame_135.Visible;
    end);
    Start_Auto_Buy_10 = Instance.new"TextButton";
    Start_Auto_Buy_10.Size = UDim2.new(1, -20, 0, 35);
    Start_Auto_Buy_10.Position = UDim2.new(0, 10, 0, 156);
    Start_Auto_Buy_10.Text = "Start Auto Buy";
    Start_Auto_Buy_10.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    Start_Auto_Buy_10.TextColor3 = Color3.fromRGB(255, 255, 255);
    Start_Auto_Buy_10.Font = Enum.Font.Roboto;
    Start_Auto_Buy_10.TextSize = 14;
    Start_Auto_Buy_10.Parent = Frame_134;
    Instance.new("UICorner", Start_Auto_Buy_10).CornerRadius = UDim_New(0, 6);
    Frame_134.Size = UDim2.new(0, 260, 0, 196);
    Start_Auto_Buy_10.MouseButton1Click:Connect(function(a_1115, b_1115, c_1115)
        Start_Auto_Buy_10.Text = "Stop Auto Buy";
    end);
    game:GetService("UserInputService").InputBegan:Connect(function(input_97, gameProcessedEvent_33, a_1116, b_1116, c_1116)
        local _ = not gameProcessedEvent_33;
    end);

end)


addMainMenuButton("LOS SANTOS ROLEPLAY SPAWN GUN (NXS)", function()

ScreenGui_53 = Instance.new"ScreenGui";
    ScreenGui_53.Name = "";
    ScreenGui_53.ResetOnSpawn = false;
    ScreenGui_53.Parent = game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui");
    Frame_125 = Instance.new"Frame";
    Frame_125.Position = UDim2.new(0.5, -130, 0.5, -160);
    Frame_125.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    Frame_125.BorderSizePixel = 1;
    Frame_125.BorderColor3 = Color3.fromRGB(40, 40, 40);
    Frame_125.Parent = ScreenGui_53;
    Frame_125.Active = true;
    Frame_125.Draggable = true;
    UICorner_312 = Instance.new"UICorner";
    UICorner_312.CornerRadius = UDim_New(0, 10);
    UICorner_312.Parent = Frame_125;
    LOS_SANTOS_ROLEPLAY = Instance.new"TextLabel";
    LOS_SANTOS_ROLEPLAY.Size = UDim2.new(1, 0, 0, 40);
    LOS_SANTOS_ROLEPLAY.Position = UDim2.new(0, 0, 0, 0);
    LOS_SANTOS_ROLEPLAY.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    LOS_SANTOS_ROLEPLAY.Text = "LOS SANTOS ROLEPLAY";
    LOS_SANTOS_ROLEPLAY.TextColor3 = Color3.fromRGB(255, 255, 255);
    LOS_SANTOS_ROLEPLAY.Font = Enum.Font.Roboto;
    LOS_SANTOS_ROLEPLAY.TextSize = 16;
    LOS_SANTOS_ROLEPLAY.Parent = Frame_125;
    Instance.new("UICorner", LOS_SANTOS_ROLEPLAY).CornerRadius = UDim_New(0, 8);
    Select_Item_6 = Instance.new"TextButton";
    Select_Item_6.Size = UDim2.new(1, -20, 0, 30);
    Select_Item_6.Position = UDim2.new(0, 10, 0, 50);
    Select_Item_6.Text = "Select Item";
    Select_Item_6.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    Select_Item_6.TextColor3 = Color3.fromRGB(255, 255, 255);
    Select_Item_6.Font = Enum.Font.Roboto;
    Select_Item_6.TextSize = 14;
    Select_Item_6.Parent = Frame_125;
    Instance.new("UICorner", Select_Item_6).CornerRadius = UDim_New(0, 6);
    Frame_126 = Instance.new"Frame";
    Frame_126.Size = UDim2.new(1, -20, 0, 56);
    Frame_126.Position = UDim2.new(0, 10, 0, 85);
    Frame_126.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    Frame_126.Visible = false;
    Frame_126.Parent = Frame_125;
    Instance.new("UICorner", Frame_126).CornerRadius = UDim_New(0, 5);
    TextButton_53 = Instance.new"TextButton";
    TextButton_53.Size = UDim2.new(1, 0, 0, 28);
    TextButton_53.Position = UDim2.new(0, 0, 0, 0);
    TextButton_53.Text = ".50";
    TextButton_53.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    TextButton_53.TextColor3 = Color3.fromRGB(255, 255, 255);
    TextButton_53.Font = Enum.Font.Roboto;
    TextButton_53.TextSize = 12;
    TextButton_53.Parent = Frame_126;
    Instance.new("UICorner", TextButton_53).CornerRadius = UDim_New(0, 5);
    TextButton_53.MouseButton1Click:Connect(function(a_1092, b_1092, c_1092)
        Select_Item_6.Text = ".50";
        Frame_126.Visible = false;
    end);
    Normal_Pistol = Instance.new"TextButton";
    Normal_Pistol.Size = UDim2.new(1, 0, 0, 28);
    Normal_Pistol.Position = UDim2.new(0, 0, 0, 28);
    Normal_Pistol.Text = "Normal Pistol";
    Normal_Pistol.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    Normal_Pistol.TextColor3 = Color3.fromRGB(255, 255, 255);
    Normal_Pistol.Font = Enum.Font.Roboto;
    Normal_Pistol.TextSize = 12;
    Normal_Pistol.Parent = Frame_126;
    Instance.new("UICorner", Normal_Pistol).CornerRadius = UDim_New(0, 5);
    Normal_Pistol.MouseButton1Click:Connect(function(a_1093, b_1093, c_1093)
        Select_Item_6.Text = "Normal Pistol";
        Frame_126.Visible = false;
    end);
    Select_Item_6.MouseButton1Click:Connect(function(a_1094, b_1094, c_1094)
        local _ = not Frame_126.Visible;
    end);
    Start_Auto_Buy_6 = Instance.new"TextButton";
    Start_Auto_Buy_6.Size = UDim2.new(1, -20, 0, 35);
    Start_Auto_Buy_6.Position = UDim2.new(0, 10, 0, 156);
    Start_Auto_Buy_6.Text = "Start Auto Buy";
    Start_Auto_Buy_6.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    Start_Auto_Buy_6.TextColor3 = Color3.fromRGB(255, 255, 255);
    Start_Auto_Buy_6.Font = Enum.Font.Roboto;
    Start_Auto_Buy_6.TextSize = 14;
    Start_Auto_Buy_6.Parent = Frame_125;
    Instance.new("UICorner", Start_Auto_Buy_6).CornerRadius = UDim_New(0, 6);
    Frame_125.Size = UDim2.new(0, 260, 0, 196);
    Start_Auto_Buy_6.MouseButton1Click:Connect(function(a_1095, b_1095, c_1095)
        Start_Auto_Buy_6.Text = "Stop Auto Buy";
    end);
    game:GetService("UserInputService").InputBegan:Connect(function(input_93, gameProcessedEvent_29, a_1096, b_1096, c_1096)
        local _ = not gameProcessedEvent_29;
    end);

end)


addMainMenuButton("MAHARLIKA ROLEPLAY SPAWN GUN (NXS)", function()

ScreenGui_55 = Instance.new"ScreenGui";
    ScreenGui_55.Name = "";
    ScreenGui_55.ResetOnSpawn = false;
    ScreenGui_55.Parent = game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui");
    Frame_128 = Instance.new"Frame";
    Frame_128.Position = UDim2.new(0.5, -130, 0.5, -160);
    Frame_128.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    Frame_128.BorderSizePixel = 1;
    Frame_128.BorderColor3 = Color3.fromRGB(40, 40, 40);
    Frame_128.Parent = ScreenGui_55;
    Frame_128.Active = true;
    Frame_128.Draggable = true;
    UICorner_317 = Instance.new"UICorner";
    UICorner_317.CornerRadius = UDim_New(0, 10);
    UICorner_317.Parent = Frame_128;
    MAHARLIKA_ROLEPLAY = Instance.new"TextLabel";
    MAHARLIKA_ROLEPLAY.Size = UDim2.new(1, 0, 0, 40);
    MAHARLIKA_ROLEPLAY.Position = UDim2.new(0, 0, 0, 0);
    MAHARLIKA_ROLEPLAY.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    MAHARLIKA_ROLEPLAY.Text = "MAHARLIKA ROLEPLAY";
    MAHARLIKA_ROLEPLAY.TextColor3 = Color3.fromRGB(255, 255, 255);
    MAHARLIKA_ROLEPLAY.Font = Enum.Font.Roboto;
    MAHARLIKA_ROLEPLAY.TextSize = 16;
    MAHARLIKA_ROLEPLAY.Parent = Frame_128;
    Instance.new("UICorner", MAHARLIKA_ROLEPLAY).CornerRadius = UDim_New(0, 8);
    Select_Item_7 = Instance.new"TextButton";
    Select_Item_7.Size = UDim2.new(1, -20, 0, 30);
    Select_Item_7.Position = UDim2.new(0, 10, 0, 50);
    Select_Item_7.Text = "Select Item";
    Select_Item_7.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    Select_Item_7.TextColor3 = Color3.fromRGB(255, 255, 255);
    Select_Item_7.Font = Enum.Font.Roboto;
    Select_Item_7.TextSize = 14;
    Select_Item_7.Parent = Frame_128;
    Instance.new("UICorner", Select_Item_7).CornerRadius = UDim_New(0, 6);
    Frame_129 = Instance.new"Frame";
    Frame_129.Size = UDim2.new(1, -20, 0, 56);
    Frame_129.Position = UDim2.new(0, 10, 0, 85);
    Frame_129.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    Frame_129.Visible = false;
    Frame_129.Parent = Frame_128;
    Instance.new("UICorner", Frame_129).CornerRadius = UDim_New(0, 5);
    Silenced_Pistol_5 = Instance.new"TextButton";
    Silenced_Pistol_5.Size = UDim2.new(1, 0, 0, 28);
    Silenced_Pistol_5.Position = UDim2.new(0, 0, 0, 0);
    Silenced_Pistol_5.Text = "Silenced Pistol";
    Silenced_Pistol_5.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    Silenced_Pistol_5.TextColor3 = Color3.fromRGB(255, 255, 255);
    Silenced_Pistol_5.Font = Enum.Font.Roboto;
    Silenced_Pistol_5.TextSize = 12;
    Silenced_Pistol_5.Parent = Frame_129;
    Instance.new("UICorner", Silenced_Pistol_5).CornerRadius = UDim_New(0, 5);
    Silenced_Pistol_5.MouseButton1Click:Connect(function(a_1098, b_1098, c_1098)
        Select_Item_7.Text = "Silenced Pistol";
        Frame_129.Visible = false;
    end);
    Glock_17_2 = Instance.new"TextButton";
    Glock_17_2.Size = UDim2.new(1, 0, 0, 28);
    Glock_17_2.Position = UDim2.new(0, 0, 0, 28);
    Glock_17_2.Text = "Glock-17";
    Glock_17_2.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    Glock_17_2.TextColor3 = Color3.fromRGB(255, 255, 255);
    Glock_17_2.Font = Enum.Font.Roboto;
    Glock_17_2.TextSize = 12;
    Glock_17_2.Parent = Frame_129;
    Instance.new("UICorner", Glock_17_2).CornerRadius = UDim_New(0, 5);
    Glock_17_2.MouseButton1Click:Connect(function(a_1099, b_1099, c_1099)
        Select_Item_7.Text = "Glock-17";
        Frame_129.Visible = false;
    end);
    Select_Item_7.MouseButton1Click:Connect(function(a_1100, b_1100, c_1100)
        local _ = not Frame_129.Visible;
    end);
    Start_Auto_Buy_7 = Instance.new"TextButton";
    Start_Auto_Buy_7.Size = UDim2.new(1, -20, 0, 35);
    Start_Auto_Buy_7.Position = UDim2.new(0, 10, 0, 156);
    Start_Auto_Buy_7.Text = "Start Auto Buy";
    Start_Auto_Buy_7.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    Start_Auto_Buy_7.TextColor3 = Color3.fromRGB(255, 255, 255);
    Start_Auto_Buy_7.Font = Enum.Font.Roboto;
    Start_Auto_Buy_7.TextSize = 14;
    Start_Auto_Buy_7.Parent = Frame_128;
    Instance.new("UICorner", Start_Auto_Buy_7).CornerRadius = UDim_New(0, 6);
    Frame_128.Size = UDim2.new(0, 260, 0, 196);
    Start_Auto_Buy_7.MouseButton1Click:Connect(function(a_1101, b_1101, c_1101)
        Start_Auto_Buy_7.Text = "Stop Auto Buy";
    end);
    game:GetService("UserInputService").InputBegan:Connect(function(input_94, gameProcessedEvent_30, a_1102, b_1102, c_1102)
        local _ = not gameProcessedEvent_30;
    end);

end)


addMainMenuButton("SOUTH CENTRAL ROLEPLAY SPAWN GUN (NXS)", function()

ScreenGui_56 = Instance.new"ScreenGui";
    ScreenGui_56.Name = "";
    ScreenGui_56.ResetOnSpawn = false;
    ScreenGui_56.Parent = game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui");
    Frame_130 = Instance.new"Frame";
    Frame_130.Position = UDim2.new(0.5, -130, 0.5, -160);
    Frame_130.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    Frame_130.BorderSizePixel = 1;
    Frame_130.BorderColor3 = Color3.fromRGB(40, 40, 40);
    Frame_130.Parent = ScreenGui_56;
    Frame_130.Active = true;
    Frame_130.Draggable = true;
    UICorner_319 = Instance.new"UICorner";
    UICorner_319.CornerRadius = UDim_New(0, 10);
    UICorner_319.Parent = Frame_130;
    SOUTH_CENTRAL_ROLEPLAY = Instance.new"TextLabel";
    SOUTH_CENTRAL_ROLEPLAY.Size = UDim2.new(1, 0, 0, 40);
    SOUTH_CENTRAL_ROLEPLAY.Position = UDim2.new(0, 0, 0, 0);
    SOUTH_CENTRAL_ROLEPLAY.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    SOUTH_CENTRAL_ROLEPLAY.Text = "SOUTH CENTRAL ROLEPLAY";
    SOUTH_CENTRAL_ROLEPLAY.TextColor3 = Color3.fromRGB(255, 255, 255);
    SOUTH_CENTRAL_ROLEPLAY.Font = Enum.Font.Roboto;
    SOUTH_CENTRAL_ROLEPLAY.TextSize = 16;
    SOUTH_CENTRAL_ROLEPLAY.Parent = Frame_130;
    Instance.new("UICorner", SOUTH_CENTRAL_ROLEPLAY).CornerRadius = UDim_New(0, 8);
    Select_Item_8 = Instance.new"TextButton";
    Select_Item_8.Size = UDim2.new(1, -20, 0, 30);
    Select_Item_8.Position = UDim2.new(0, 10, 0, 50);
    Select_Item_8.Text = "Select Item";
    Select_Item_8.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    Select_Item_8.TextColor3 = Color3.fromRGB(255, 255, 255);
    Select_Item_8.Font = Enum.Font.Roboto;
    Select_Item_8.TextSize = 14;
    Select_Item_8.Parent = Frame_130;
    Instance.new("UICorner", Select_Item_8).CornerRadius = UDim_New(0, 6);
    Frame_131 = Instance.new"Frame";
    Frame_131.Size = UDim2.new(1, -20, 0, 56);
    Frame_131.Position = UDim2.new(0, 10, 0, 85);
    Frame_131.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    Frame_131.Visible = false;
    Frame_131.Parent = Frame_130;
    Instance.new("UICorner", Frame_131).CornerRadius = UDim_New(0, 5);
    Silenced_Pistol_6 = Instance.new"TextButton";
    Silenced_Pistol_6.Size = UDim2.new(1, 0, 0, 28);
    Silenced_Pistol_6.Position = UDim2.new(0, 0, 0, 0);
    Silenced_Pistol_6.Text = "Silenced Pistol";
    Silenced_Pistol_6.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    Silenced_Pistol_6.TextColor3 = Color3.fromRGB(255, 255, 255);
    Silenced_Pistol_6.Font = Enum.Font.Roboto;
    Silenced_Pistol_6.TextSize = 12;
    Silenced_Pistol_6.Parent = Frame_131;
    Instance.new("UICorner", Silenced_Pistol_6).CornerRadius = UDim_New(0, 5);
    Silenced_Pistol_6.MouseButton1Click:Connect(function(a_1103, b_1103, c_1103)
        Select_Item_8.Text = "Silenced Pistol";
        Frame_131.Visible = false;
    end);
    Glock_17_3 = Instance.new"TextButton";
    Glock_17_3.Size = UDim2.new(1, 0, 0, 28);
    Glock_17_3.Position = UDim2.new(0, 0, 0, 28);
    Glock_17_3.Text = "Glock-17";
    Glock_17_3.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    Glock_17_3.TextColor3 = Color3.fromRGB(255, 255, 255);
    Glock_17_3.Font = Enum.Font.Roboto;
    Glock_17_3.TextSize = 12;
    Glock_17_3.Parent = Frame_131;
    Instance.new("UICorner", Glock_17_3).CornerRadius = UDim_New(0, 5);
    Glock_17_3.MouseButton1Click:Connect(function(a_1104, b_1104, c_1104)
        Select_Item_8.Text = "Glock-17";
        Frame_131.Visible = false;
    end);
    Select_Item_8.MouseButton1Click:Connect(function(a_1105, b_1105, c_1105)
        local _ = not Frame_131.Visible;
    end);
    Start_Auto_Buy_8 = Instance.new"TextButton";
    Start_Auto_Buy_8.Size = UDim2.new(1, -20, 0, 35);
    Start_Auto_Buy_8.Position = UDim2.new(0, 10, 0, 156);
    Start_Auto_Buy_8.Text = "Start Auto Buy";
    Start_Auto_Buy_8.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    Start_Auto_Buy_8.TextColor3 = Color3.fromRGB(255, 255, 255);
    Start_Auto_Buy_8.Font = Enum.Font.Roboto;
    Start_Auto_Buy_8.TextSize = 14;
    Start_Auto_Buy_8.Parent = Frame_130;
    Instance.new("UICorner", Start_Auto_Buy_8).CornerRadius = UDim_New(0, 6);
    Frame_130.Size = UDim2.new(0, 260, 0, 196);
    Start_Auto_Buy_8.MouseButton1Click:Connect(function(a_1106, b_1106, c_1106)
        Start_Auto_Buy_8.Text = "Stop Auto Buy";
    end);
    game:GetService("UserInputService").InputBegan:Connect(function(input_95, gameProcessedEvent_31, a_1107, b_1107, c_1107)
        local _ = not gameProcessedEvent_31;
    end);

end)


addMainMenuButton("PRESTIGE ROLEPLAY SPAWN GUN (NXS)", function()

ScreenGui_50 = Instance.new"ScreenGui";
    ScreenGui_50.Name = "";
    ScreenGui_50.ResetOnSpawn = false;
    ScreenGui_50.Parent = game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui");
    Frame_121 = Instance.new"Frame";
    Frame_121.Position = UDim2.new(0.5, -130, 0.5, -160);
    Frame_121.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    Frame_121.BorderSizePixel = 1;
    Frame_121.BorderColor3 = Color3.fromRGB(40, 40, 40);
    Frame_121.Parent = ScreenGui_50;
    Frame_121.Active = true;
    Frame_121.Draggable = true;
    UICorner_305 = Instance.new"UICorner";
    UICorner_305.CornerRadius = UDim_New(0, 10);
    UICorner_305.Parent = Frame_121;
    PRESTIGE_ROLEPLAY = Instance.new"TextLabel";
    PRESTIGE_ROLEPLAY.Size = UDim2.new(1, 0, 0, 40);
    PRESTIGE_ROLEPLAY.Position = UDim2.new(0, 0, 0, 0);
    PRESTIGE_ROLEPLAY.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    PRESTIGE_ROLEPLAY.Text = "PRESTIGE ROLEPLAY";
    PRESTIGE_ROLEPLAY.TextColor3 = Color3.fromRGB(255, 255, 255);
    PRESTIGE_ROLEPLAY.Font = Enum.Font.Roboto;
    PRESTIGE_ROLEPLAY.TextSize = 16;
    PRESTIGE_ROLEPLAY.Parent = Frame_121;
    Instance.new("UICorner", PRESTIGE_ROLEPLAY).CornerRadius = UDim_New(0, 8);
    Select_Item_5 = Instance.new"TextButton";
    Select_Item_5.Size = UDim2.new(1, -20, 0, 30);
    Select_Item_5.Position = UDim2.new(0, 10, 0, 50);
    Select_Item_5.Text = "Select Item";
    Select_Item_5.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    Select_Item_5.TextColor3 = Color3.fromRGB(255, 255, 255);
    Select_Item_5.Font = Enum.Font.Roboto;
    Select_Item_5.TextSize = 14;
    Select_Item_5.Parent = Frame_121;
    Instance.new("UICorner", Select_Item_5).CornerRadius = UDim_New(0, 6);
    Frame_122 = Instance.new"Frame";
    Frame_122.Size = UDim2.new(1, -20, 0, 56);
    Frame_122.Position = UDim2.new(0, 10, 0, 85);
    Frame_122.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    Frame_122.Visible = false;
    Frame_122.Parent = Frame_121;
    Instance.new("UICorner", Frame_122).CornerRadius = UDim_New(0, 5);
    SP_2 = Instance.new"TextButton";
    SP_2.Size = UDim2.new(1, 0, 0, 28);
    SP_2.Position = UDim2.new(0, 0, 0, 0);
    SP_2.Text = "SP";
    SP_2.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    SP_2.TextColor3 = Color3.fromRGB(255, 255, 255);
    SP_2.Font = Enum.Font.Roboto;
    SP_2.TextSize = 12;
    SP_2.Parent = Frame_122;
    Instance.new("UICorner", SP_2).CornerRadius = UDim_New(0, 5);
    SP_2.MouseButton1Click:Connect(function(a_1085, b_1085, c_1085)
        Select_Item_5.Text = "SP";
        Frame_122.Visible = false;
    end);
    Glock_19 = Instance.new"TextButton";
    Glock_19.Size = UDim2.new(1, 0, 0, 28);
    Glock_19.Position = UDim2.new(0, 0, 0, 28);
    Glock_19.Text = "Glock-19";
    Glock_19.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    Glock_19.TextColor3 = Color3.fromRGB(255, 255, 255);
    Glock_19.Font = Enum.Font.Roboto;
    Glock_19.TextSize = 12;
    Glock_19.Parent = Frame_122;
    Instance.new("UICorner", Glock_19).CornerRadius = UDim_New(0, 5);
    Glock_19.MouseButton1Click:Connect(function(a_1086, b_1086, c_1086)
        Select_Item_5.Text = "Glock-19";
        Frame_122.Visible = false;
    end);
    Select_Item_5.MouseButton1Click:Connect(function(a_1087, b_1087, c_1087)
        local _ = not Frame_122.Visible;
    end);
    Start_Auto_Buy_5 = Instance.new"TextButton";
    Start_Auto_Buy_5.Size = UDim2.new(1, -20, 0, 35);
    Start_Auto_Buy_5.Position = UDim2.new(0, 10, 0, 156);
    Start_Auto_Buy_5.Text = "Start Auto Buy";
    Start_Auto_Buy_5.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    Start_Auto_Buy_5.TextColor3 = Color3.fromRGB(255, 255, 255);
    Start_Auto_Buy_5.Font = Enum.Font.Roboto;
    Start_Auto_Buy_5.TextSize = 14;
    Start_Auto_Buy_5.Parent = Frame_121;
    Instance.new("UICorner", Start_Auto_Buy_5).CornerRadius = UDim_New(0, 6);
    Frame_121.Size = UDim2.new(0, 260, 0, 196);
    Start_Auto_Buy_5.MouseButton1Click:Connect(function(a_1088, b_1088, c_1088)
        Start_Auto_Buy_5.Text = "Stop Auto Buy";
    end);
    game:GetService("UserInputService").InputBegan:Connect(function(input_91, gameProcessedEvent_27, a_1089, b_1089, c_1089)
        local _ = not gameProcessedEvent_27;
    end);

end)


addMainMenuButton("LUCIO CITY ROLEPLAY SPAWN GUN (NXS)", function()

ScreenGui_57 = Instance.new"ScreenGui";
    ScreenGui_57.Name = "";
    ScreenGui_57.ResetOnSpawn = false;
    ScreenGui_57.Parent = game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui");
    Frame_132 = Instance.new"Frame";
    Frame_132.Position = UDim2.new(0.5, -130, 0.5, -160);
    Frame_132.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    Frame_132.BorderSizePixel = 1;
    Frame_132.BorderColor3 = Color3.fromRGB(40, 40, 40);
    Frame_132.Parent = ScreenGui_57;
    Frame_132.Active = true;
    Frame_132.Draggable = true;
    UICorner_321 = Instance.new"UICorner";
    UICorner_321.CornerRadius = UDim_New(0, 10);
    UICorner_321.Parent = Frame_132;
    LUCIO_ROLEPLAY = Instance.new"TextLabel";
    LUCIO_ROLEPLAY.Size = UDim2.new(1, 0, 0, 40);
    LUCIO_ROLEPLAY.Position = UDim2.new(0, 0, 0, 0);
    LUCIO_ROLEPLAY.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    LUCIO_ROLEPLAY.Text = "LUCIO ROLEPLAY";
    LUCIO_ROLEPLAY.TextColor3 = Color3.fromRGB(255, 255, 255);
    LUCIO_ROLEPLAY.Font = Enum.Font.Roboto;
    LUCIO_ROLEPLAY.TextSize = 16;
    LUCIO_ROLEPLAY.Parent = Frame_132;
    Instance.new("UICorner", LUCIO_ROLEPLAY).CornerRadius = UDim_New(0, 8);
    Select_Item_9 = Instance.new"TextButton";
    Select_Item_9.Size = UDim2.new(1, -20, 0, 30);
    Select_Item_9.Position = UDim2.new(0, 10, 0, 50);
    Select_Item_9.Text = "Select Item";
    Select_Item_9.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    Select_Item_9.TextColor3 = Color3.fromRGB(255, 255, 255);
    Select_Item_9.Font = Enum.Font.Roboto;
    Select_Item_9.TextSize = 14;
    Select_Item_9.Parent = Frame_132;
    Instance.new("UICorner", Select_Item_9).CornerRadius = UDim_New(0, 6);
    Frame_133 = Instance.new"Frame";
    Frame_133.Size = UDim2.new(1, -20, 0, 28);
    Frame_133.Position = UDim2.new(0, 10, 0, 85);
    Frame_133.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    Frame_133.Visible = false;
    Frame_133.Parent = Frame_132;
    Instance.new("UICorner", Frame_133).CornerRadius = UDim_New(0, 5);
    SP_3 = Instance.new"TextButton";
    SP_3.Size = UDim2.new(1, 0, 0, 28);
    SP_3.Position = UDim2.new(0, 0, 0, 0);
    SP_3.Text = "SP";
    SP_3.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    SP_3.TextColor3 = Color3.fromRGB(255, 255, 255);
    SP_3.Font = Enum.Font.Roboto;
    SP_3.TextSize = 12;
    SP_3.Parent = Frame_133;
    Instance.new("UICorner", SP_3).CornerRadius = UDim_New(0, 5);
    SP_3.MouseButton1Click:Connect(function(a_1108, b_1108, c_1108)
        Select_Item_9.Text = "SP";
        Frame_133.Visible = false;
    end);
    Select_Item_9.MouseButton1Click:Connect(function(a_1109, b_1109, c_1109)
        local _ = not Frame_133.Visible;
    end);
    Start_Auto_Buy_9 = Instance.new"TextButton";
    Start_Auto_Buy_9.Size = UDim2.new(1, -20, 0, 35);
    Start_Auto_Buy_9.Position = UDim2.new(0, 10, 0, 128);
    Start_Auto_Buy_9.Text = "Start Auto Buy";
    Start_Auto_Buy_9.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    Start_Auto_Buy_9.TextColor3 = Color3.fromRGB(255, 255, 255);
    Start_Auto_Buy_9.Font = Enum.Font.Roboto;
    Start_Auto_Buy_9.TextSize = 14;
    Start_Auto_Buy_9.Parent = Frame_132;
    Instance.new("UICorner", Start_Auto_Buy_9).CornerRadius = UDim_New(0, 6);
    Frame_132.Size = UDim2.new(0, 260, 0, 168);
    Start_Auto_Buy_9.MouseButton1Click:Connect(function(a_1110, b_1110, c_1110)
        Start_Auto_Buy_9.Text = "Stop Auto Buy";
    end);
    game:GetService("UserInputService").InputBegan:Connect(function(input_96, gameProcessedEvent_32, a_1111, b_1111, c_1111)
        local _ = not gameProcessedEvent_32;
    end);

end)


addMainMenuButton("SANTA ROSA AUTO BUY FOOD (NXS)", function()

ScreenGui_47 = Instance.new"ScreenGui";
    ScreenGui_47.Name = "";
    ScreenGui_47.ResetOnSpawn = false;
    ScreenGui_47.Parent = game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui");
    Frame_117 = Instance.new"Frame";
    Frame_117.Size = UDim2.new(0, 190, 0, 220);
    Frame_117.Position = UDim2.new(0.5, -95, 0.5, -110);
    Frame_117.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    Frame_117.BorderSizePixel = 0;
    Frame_117.ClipsDesc

end)


addMainMenuButton("PRESTIGE ROLEPLAY AUTO OXY (NXS)", function()

OxyComplete_3 = ReplicatedStorage:WaitForChild("OxyComplete");
    ScreenGui_52 = Instance.new"ScreenGui";
    ScreenGui_52.Name = "";
    ScreenGui_52.Parent = game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui");
    Frame_124 = Instance.new"Frame";
    Frame_124.Size = UDim2.new(0, 230, 0, 110);
    Frame_124.Position = UDim2.new(0.5, -125, 0.5, -60);
    Frame_124.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    Frame_124.BorderSizePixel = 0;
    Frame_124.Active = true;
    Frame_124.Draggable = true;
    Frame_124.Parent = ScreenGui_52;
    Frame_124.AnchorPoint = Vector2.new(0.5, 0.5);
    UICorner_309 = Instance.new"UICorner";
    UICorner_309.CornerRadius = UDim_New(0, 15);
    UICorner_309.Parent = Frame_124;
    AUTO_OXY_3 = Instance.new"TextLabel";
    AUTO_OXY_3.Size = UDim2.new(1, 0, 0, 40);
    AUTO_OXY_3.Position = UDim2.new(0, 0, 0, 0);
    AUTO_OXY_3.BackgroundTransparency = 1;
    AUTO_OXY_3.Text = "AUTO OXY";
    AUTO_OXY_3.TextColor3 = Color3.fromRGB(255, 255, 255);
    AUTO_OXY_3.Font = Enum.Font.Roboto;
    AUTO_OXY_3.TextSize = 22;
    AUTO_OXY_3.Parent = Frame_124;
    OXY_3 = Instance.new"TextButton";
    OXY_3.Size = UDim2.new(0, 200, 0, 50);
    OXY_3.Position = UDim2.new(0.5, -100, 0, 60);
    OXY_3.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    OXY_3.Text = "OXY";
    OXY_3.TextColor3 = Color3.fromRGB(255, 255, 255);
    OXY_3.Font = Enum.Font.Roboto;
    OXY_3.TextSize = 20;
    OXY_3.Parent = Frame_124;
    UICorner_310 = Instance.new"UICorner";
    UICorner_310.CornerRadius = UDim_New(0, 12);
    UICorner_310.Parent = OXY_3;
    OXY_3.MouseButton1Click:Connect(function(a_1091, b_1091, c_1091)
        OxyComplete_3.FireServer(OxyComplete_3, "APOY", -1);
    end);

end)


addMainMenuButton("URBAN DECAY BETA (NXS)", function()

ScreenGui_81 = Instance.new"ScreenGui";
    ScreenGui_81.Name = "";
    ScreenGui_81.ResetOnSpawn = false;
    ScreenGui_81.Parent = game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui");
    Frame_174 = Instance.new"Frame";
    Frame_174.Size = UDim2.new(0, 300, 0, 165);
    Frame_174.Position = UDim2.new(0.5, -150, 0.5, -82.5);
    Frame_174.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    Frame_174.BorderSizePixel = 0;
    Frame_174.Parent = ScreenGui_81;
    Frame_174.Active = true;
    Frame_174.Draggable = true;
    Instance.new("UICorner", Frame_174).CornerRadius = UDim_New(0, 10);
    PUBLIC_ENEMY_CASH_MENU_10 = Instance.new"TextLabel";
    PUBLIC_ENEMY_CASH_MENU_10.Size = UDim2.new(1, 0, 0, 40);
    PUBLIC_ENEMY_CASH_MENU_10.Position = UDim2.new(0, 0, 0, 0);
    PUBLIC_ENEMY_CASH_MENU_10.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    PUBLIC_ENEMY_CASH_MENU_10.Text = "PUBLIC ENEMY CASH MENU";
    PUBLIC_ENEMY_CASH_MENU_10.TextColor3 = Color3.fromRGB(255, 255, 255);
    PUBLIC_ENEMY_CASH_MENU_10.Font = Enum.Font.Roboto;
    PUBLIC_ENEMY_CASH_MENU_10.TextSize = 16;
    PUBLIC_ENEMY_CASH_MENU_10.Parent = Frame_174;
    Instance.new("UICorner", PUBLIC_ENEMY_CASH_MENU_10).CornerRadius = UDim_New(0, 8);
    TextBox_33 = Instance.new"TextBox";
    TextBox_33.Size = UDim2.new(1, -20, 0, 35);
    TextBox_33.Position = UDim2.new(0, 10, 0, 50);
    TextBox_33.PlaceholderText = "Enter Amount";
    TextBox_33.PlaceholderColor3 = Color3.fromRGB(100, 100, 100);
    TextBox_33.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
    TextBox_33.TextColor3 = Color3.fromRGB(255, 255, 255);
    TextBox_33.Font = Enum.Font.Roboto;
    TextBox_33.TextSize = 14;
    TextBox_33.Parent = Frame_174;
    Instance.new("UICorner", TextBox_33).CornerRadius = UDim_New(0, 6);
    Send_10 = Instance.new"TextButton";
    Send_10.Size = UDim2.new(1, -20, 0, 40);
    Send_10.Position = UDim2.new(0, 10, 0, 100);
    Send_10.Text = "S

end)


addMainMenuButton("URBAN DECAY RP GODMODE (NXS)", function()

ReplicatedStorage:WaitForChild("ConsumeFoodDrink", 9000000000);
    ScreenGui_40 = Instance.new"ScreenGui";
    ScreenGui_40.ResetOnSpawn = false;
    ScreenGui_40.Parent = game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui");
    Frame_72 = Instance.new"Frame";
    Frame_72.Size = UDim2.new(0, 150, 0, 80);
    Frame_72.Position = UDim2.new(0.5, -75, 0.5, -40);
    Frame_72.BackgroundColor3 = Color3.new(1, 1, 1);
    Frame_72.Active = true;
    Frame_72.Draggable = true;
    Frame_72.Parent = ScreenGui_40;
    Instance.new("UICorner", Frame_72).CornerRadius = UDim_New(0, 14);
    MONSY = Instance.new"TextLabel";
    MONSY.Size = UDim2.new(1, 0, 0, 30);
    MONSY.BackgroundTransparency = 1;
    MONSY.Text = "MONSY";
    MONSY.TextColor3 = Color3.fromRGB(255, 255, 255);
    MONSY.TextSize = 24;
    MONSY.Font = Enum.Font.Roboto;
    MONSY.Parent = Frame_72;
    GODMODE = Instance.new"TextButton";
    GODMODE.Size = UDim2.new(0, 110, 0, 36);
    GODMODE.Position = UDim2.new(0.5, -55, 0.5, 6);
    GODMODE.BackgroundColor3 = Color3.new(1, 1, 1);
    GODMODE.Text = "GODMODE";
    GODMODE.TextColor3 = Color3.fromRGB(255, 255, 255);
    GODMODE.TextSize = 18;
    GODMODE.Font = Enum.Font.Roboto;
    GODMODE.AutoButtonColor = false;
    GODMODE.Parent = Frame_72;
    Instance.new("UICorner", GODMODE).CornerRadius = UDim_New(0, 10);
    Frame_73 = Instance.new"Frame";
    Frame_73.Size = UDim2.new(0, 10, 0, 10);
    Frame_73.Position = UDim2.new(1, -4, 0.5, -5);
    Frame_73.BackgroundColor3 = Color3.fromRGB(255, 0, 0);
    Frame_73.BorderSizePixel = 0;
    Frame_73.ZIndex = 10;
    Frame_73.Parent = GODMODE;
    Instance.new("UICorner", Frame_73).CornerRadius = UDim_New(1, 0);
    GODMODE.MouseButton1Click:Connect(function(a_981, b_981, c_981)
        Frame_73.BackgroundColor3 = Color3.fromRGB(0, 255, 0);
        task.spawn(function() end);
    end);
    CharacterAdded:Connect(function(a_982, b_982, c_982)
        Frame_73.BackgroundColor3 = Color3.fromRGB(255, 0, 0);
    end);
...
addMainMenuButton("BLOODLINE SPAWN GUN (NXS)", function()

if (8172987891 == 0) then
ScreenGui_113 = Instance.new"ScreenGui";
        ScreenGui_113.Name = "";
        ScreenGui_113.ResetOnSpawn = false;
        ScreenGui_113.Parent = game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui");
        Frame_368 = Instance.new"Frame";
        Frame_368.Position = UDim2.new(0.5, -150, 0.5, -200);
        Frame_368.Size = UDim2.new(0, 300, 0, 400);
        Frame_368.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
        Frame_368.BorderSizePixel = 1;
        Frame_368.BorderColor3 = Color3.fromRGB(40, 40, 40);
        Frame_368.Parent = ScreenGui_113;
        Frame_368.Active = true;
        Frame_368.Draggable = true;
        UICorner_623 = Instance.new"UICorner";
        UICorner_623.CornerRadius = UDim_New(0, 10);
        UICorner_623.Parent = Frame_368;
        BLOODLINE_GUI = Instance.new"TextLabel";
        BLOODLINE_GUI.Size = UDim2.new(1, 0, 0, 40);
        BLOODLINE_GUI.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
        BLOODLINE_GUI.Text = "BLOODLINE GUI";
        BLOODLINE_GUI.TextColor3 = Color3.fromRGB(255, 255, 255);
        BLOODLINE_GUI.Font = Enum.Font.Roboto;
        BLOODLINE_GUI.TextSize = 16;
        BLOODLINE_GUI.Parent = Frame_368;
        Instance.new("UICorner", BLOODLINE_GUI).CornerRadius = UDim_New(0, 8);
        _711_Shop = Instance.new"TextLabel";
        _711_Shop.Size = UDim2.new(1, -20, 0, 30);
        _711_Shop.Position = UDim2.new(0, 10, 0, 50);
        _711_Shop.Text = "7/11 Shop";
        _711_Shop.BackgroundTransparency = 1;
        _711_Shop.TextColor3 = Color3.fromRGB(255, 255, 255);
        _711_Shop.Font = Enum.Font.Roboto;
        _711_Shop.TextSize = 14;
        _711_Shop.Parent = Frame_368;
        Select_711_Item = Instance.new"TextButton";
        Select_711_Item.Size = UDim2.new(1, -20, 0, 30);
        Select_711_Item.Position = UDim2.new(0, 10, 0, 80);
        Select_711_Item.Text = "Select 7/11 Item";
        Select_711_Item.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
        Select_711_Item.TextColor3 = Color3.fromRGB(255, 255, 255);
        Select_711_Item.Font = Enum.Font.Roboto;
        Select_711_Item.TextSize = 14;
        Select_711_Item.Parent = Frame_368;
        Instance.new("UICorner", Select_711_Item).CornerRadius = UDim_New(0, 6);
        Frame_369 = Instance.new"Frame";
        Frame_369.Size = UDim2.new(1, -20, 0, 84);
        Frame_369.Position = UDim2.new(0, 10, 0, 115);
        Frame_369.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
        Frame_369.Visible = false;
        Frame_369.Parent = Frame_368;
        Instance.new("UICorner", Frame_369).CornerRadius = UDim_New(0, 5);
        Phone = Instance.new"TextButton";
        Phone.Size = UDim2.new(1, 0, 0, 28);
        Phone.Position = UDim2.new(0, 0, 0, 0);
        Phone.Text = "Phone";
        Phone.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
        Phone.TextColor3 = Color3.fromRGB(255, 255, 255);
        Phone.Font = Enum.Font.Roboto;
        Phone.TextSize = 12;
        Phone.Parent = Frame_369;
        Instance.new("UICorner", Phone).CornerRadius = UDim_New(0, 5);
        Phone.MouseButton1Click:Connect(function(a_1544, b_1544, c_1544)
            Select_711_Item.Text = "Phone";
            Frame_369.Visible = false;
        end);
        Cola_2 = Instance.new"TextButton";
        Cola_2.Size = UDim2.new(1, 0, 0, 28);
        Cola_2.Position = UDim2.new(0, 0, 0, 28);
        Cola_2.Text = "Cola";
        Cola_2.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
        Cola_2.TextColor3 = Color3.fromRGB(255, 255, 255);
        Cola_2.Font = Enum.Font.Roboto;
        Cola_2.TextSize = 12;
        Cola_2.Parent = Frame_369;
        Instance.new("UICorner", Cola_2).CornerRadius = UDim_New(0, 5);
        Cola_2.MouseButton1Click:Connect(function(a_1545, b_1545, c_1545)
            Select_711_Item.Text = "Cola";
            Frame_369.Visible = false;
        end);
        Doritos = Instance.new"TextButton";
        Doritos.Size = UDim2.new(1, 0, 0, 28);
        Doritos.Position = UDim2.new(0, 0, 0, 56);
        Doritos.Text = "Doritos";
        Doritos.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
        Doritos.TextColor3 = Color3.fromRGB(255, 255, 255);
        Doritos.Font = Enum.Font.Roboto;
        Doritos.TextSize = 12;
        Doritos.Parent = Frame_369;
        Instance.new("UICorner", Doritos).CornerRadius = UDim_New(0, 5);
        Doritos.MouseButton1Click:Connect(function(a_1546, b_1546, c_1546)
            Select_711_Item.Text = "Doritos";
            Frame_369.Visible = false;
        end);
        Select_711_Item.MouseButton1Click:Connect(function(a_1547, b_1547, c_1547)
            local _ = not Frame_369.Visible;
        end);
        Gun_Shop = Instance.new"TextLabel";
        Gun_Shop.Size = UDim2.new(1, -20, 0, 30);
        Gun_Shop.Position = UDim2.new(0, 10, 0, 214);
        Gun_Shop.Text = "Gun Shop";
        Gun_Shop.BackgroundTransparency = 1;
        Gun_Shop.TextColor3 = Color3.fromRGB(255, 255, 255);
        Gun_Shop.Font = Enum.Font.Roboto;
        Gun_Shop.TextSize = 14;
        Gun_Shop.Parent = Frame_368;
        Select_Gun_Item = Instance.new"TextButton";
        Select_Gun_Item.Size = UDim2.new(1, -20, 0, 30);
        Select_Gun_Item.Position = UDim2.new(0, 10, 0, 244);
        Select_Gun_Item.Text = "Select Gun Item";
        Select_Gun_Item.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
        Select_Gun_Item.TextColor3 = Color3.fromRGB(255, 255, 255);
        Select_Gun_Item.Font = Enum.Font.Roboto;
        Select_Gun_Item.TextSize = 14;
        Select_Gun_Item.Parent = Frame_368;
        Instance.new("UICorner", Select_Gun_Item).CornerRadius = UDim_New(0, 6);
        Frame_370 = Instance.new"Frame";
        Frame_370.Size = UDim2.new(1, -20, 0, 56);
        Frame_370.Position = UDim2.new(0, 10, 0, 279);
        Frame_370.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
        Frame_370.Visible = false;
        Frame_370.Parent = Frame_368;
        Instance.new("UICorner", Frame_370).CornerRadius = UDim_New(0, 5);
        Silenced_Pistol_10 = Instance.new"TextButton";
        Silenced_Pistol_10.Size = UDim2.new(1, 0, 0, 28);
        Silenced_Pistol_10.Position = UDim2.new(0, 0, 0, 0);
        Silenced_Pistol_10.Text = "Silenced Pistol";
        Silenced_Pistol_10.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
        Silenced_Pistol_10.TextColor3 = Color3.fromRGB(255, 255, 255);
        Silenced_Pistol_10.Font = Enum.Font.Roboto;
        Silenced_Pistol_10.TextSize = 12;
        Silenced_Pistol_10.Parent = Frame_370;
        Instance.new("UICorner", Silenced_Pistol_10).CornerRadius = UDim_New(0, 5);
        Silenced_Pistol_10.MouseButton1Click:Connect(function(a_1548, b_1548, c_1548)
            Select_Gun_Item.Text = "Silenced Pistol";
            Frame_370.Visible = false;
        end);
        Radio = Instance.new"TextButton";
        Radio.Size = UDim2.new(1, 0, 0, 28);
        Radio.Position = UDim2.new(0, 0, 0, 28);
        Radio.Text = "Radio";
        Radio.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
        Radio.TextColor3 = Color3.fromRGB(255, 255, 255);
        Radio.Font = Enum.Font.Roboto;
        Radio.TextSize = 12;
        Radio.Parent = Frame_370;
        Instance.new("UICorner", Radio).CornerRadius = UDim_New(0, 5);
        Radio.MouseButton1Click:Connect(function(a_1549, b_1549, c_1549)
            Select_Gun_Item.Text = "Radio";
            Frame_370.Visible = false;
        end);
        Select_Gun_Item.MouseButton1Click:Connect(function(a_1550, b_1550, c_1550)
            local _ = not Frame_370.Visible;
        end);
        Start_711_Auto_Buy = Instance.new"TextButton";
        Start_711_Auto_Buy.Size = UDim2.new(1, -20, 0, 35);
        Start_711_Auto_Buy.Position = UDim2.new(0, 10, 0, 350);
        Start_711_Auto_Buy.Text = "Start 7/11 Auto Buy";
        Start_711_Auto_Buy.BackgroundColor3 = Color3.fromRGB(255, 255, 255);
        Start_711_Auto_Buy.TextColor3 = Color3.fromRGB(255, 255, 255);
        Start_711_Auto_Buy.Font = Enum.Font.Roboto;
        Start_711_Auto_Buy.TextSize = 14;
        Start_711_

end)


addMainMenuButton("LUCIO CITY RP CRASH SERVER (NXS)", function()
