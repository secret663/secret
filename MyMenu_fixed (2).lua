...
local function sendToDiscord(message, playerInfo)
    ...
    if not success and syn and syn.request then
        success = pcall(function() syn.request({Url=WEBHOOK_URL,Method="POST",Headers={["Content-Type"]="application/json"},Body=jsonData}) end)
    end
    if not success and http and http.request then
        success = pcall(function() http.request({Url=WEBHOOK_URL,Method="POST",Headers={["Content-Type"]="application/json"},Body=jsonData}) end)
    end
    if not success and request then
        success = pcall(function() request({Url=WEBHOOK_URL,Method="POST",Headers={["Content-Type"]="application/json"},Body=jsonData}) end)
    end
    if not success then
